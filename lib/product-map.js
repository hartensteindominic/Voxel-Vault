...
export const APP_SECTIONS = Object.freeze([
  ...
  { id: 'create', eyebrow: 'CREATE + COLLECT', title: 'Digital assets', description: 'Create and organize digital assets first. Wallets and minting stay optional.', items: [
    { id: 'property', href: '/property', label: 'VoxelPop Property', icon: '+', description: 'Authorized photo → $4.99 local VoxelPop image + interactive 3D preview → approve → 3D voxel → optional mint → World.', badge: 'LIVE' },
    { id: 'bought-estates', href: '/vault/estates/mine', label: 'Bought Properties', icon: '◇', description: 'Open a secured Digital Estate purchase, create its interactive 3D voxel, and save that creation to your Vault with no second creation charge.', badge: 'YOUR PURCHASES' },
  ...
  { id: 'property-money', eyebrow: 'SANDBOX + VERIFIED PROVIDERS', title: 'Money + real property', description: 'These tools stay separate because a demo, financial position, lease, deed and NFT are not the same thing. Live financial or property rights require verified providers, eligibility and legal rails.', items: [
    ...
    { id: 'twins', href: '/vault/estates/mine', label: 'Digital Twins', icon: '◇', description: 'See account-secured Digital Estate purchases, create their 3D voxels, and manage optional wallet minting.', badge: 'DIGITAL' },
  ]},
...
export function isSimplePropertyRoute(pathname = '/') {
  ...
  return path === '/' || path === '/property' || path.startsWith('/property/') || path === '/geo/slice' || path.startsWith('/geo/slice/') || path === '/world' || path.startsWith('/world/') || path === '/vault' || path === '/vault/property-drafts' || path.startsWith('/vault/property-drafts/') || path === '/vault/estates/mine' || path.startsWith('/vault/estates/mine/') || path === '/vault/rentals' || path.startsWith('/vault/rentals/') || path === '/more';
}
