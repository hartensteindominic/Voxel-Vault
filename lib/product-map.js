...
export const APP_SECTIONS = Object.freeze([
  { id: 'explore', eyebrow: 'WORLD + PLACES', title: 'Explore real places', description: 'Map and 3D evidence describe a place. They do not prove ownership.', items: [
  ...
  { id: 'create', eyebrow: 'CREATE + COLLECT', title: 'Digital assets', description: 'Create and organize digital assets first. Wallets and minting stay optional.', items: [
    { id: 'property', href: '/property', label: 'VoxelPop Property', icon: '+', description: 'Authorized photo → $4.99 digital creation → local voxel → source-backed mapped 3D → World.', badge: '$4.99 DIGITAL' },
  ...
  { id: 'property-money', eyebrow: 'OPTIONAL PROPERTY + MONEY TOOLS', title: 'Sandbox, providers + title', description: 'A sandbox, provider-backed position, lease, deed, and NFT stay legally separate even when they appear in one app.', items: [
    { id: 'property-slice', href: '/geo/slice', label: '$1.99 Property Sandbox', icon: '¢', description: 'Compare tiny hypothetical property slices with fake demo balances only. No real funds or property rights move.', badge: 'DEMO' },
    { id: 'rentals', href: '/vault/rentals', label: 'Rented', icon: '⌂', description: 'View lease and payment records only when the required evidence exists.', badge: 'VERIFIED' },
    { id: 'invest', href: '/real-estate/reits', label: 'Provider investments', icon: '$', description: 'Browse provider-backed real-estate securities and sandbox/live states. Live execution stays provider- and eligibility-gated.', badge: 'PARTNER' },
    { id: 'income', href: '/vault/income', label: 'Observed income', icon: '↗', description: 'See provider-observed payment history without invented yield.', badge: 'PROVIDER' },
    { id: 'acquire', href: '/real-estate/acquire', label: 'Direct ownership path', icon: '⌂', description: 'Plan normal diligence, closing, and recorded title toward actual real-property ownership.', badge: 'TITLE' },
    { id: 'verify', href: '/vault/properties/claim', label: 'Verify property', icon: '✓', description: 'Create a Property Passport downstream of evidence; it is not a deed.', badge: 'EVIDENCE' },
...
export const APP_USER_PREFIXES = Object.freeze(['/property', '/world', '/geo', '/vault', '/studio', '/capture', '/receipt', '/avatar', '/room', '/trade', '/asset', '/marketplace', '/discover', '/ai', '/ai-licensing', '/hunt', '/real-estate', '/forge', '/purchases', '/more', '/admin']);

export function isSimplePropertyRoute(pathname = '/') {
  const path = String(pathname || '/');
  return path === '/' || path === '/property' || path.startsWith('/property/') || path === '/geo/slice' || path.startsWith('/geo/slice/') || path === '/world' || path.startsWith('/world/') || path === '/vault/property-drafts' || path.startsWith('/vault/property-drafts/') || path === '/vault/rentals' || path.startsWith('/vault/rentals/');
}
