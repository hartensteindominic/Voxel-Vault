...
async function ensureStorageReady() {
  if (!storageReadyPromise) {
    storageReadyPromise = initializeStorage()
      .then((supabase) => {
        // A missing/temporarily unavailable Storage backend must not be cached
        // for the lifetime of a warm server process. Clear the memoized promise
        // so the next status poll or save can repair itself automatically.
        if (!supabase) storageReadyPromise = undefined;
        return supabase;
      })
      .catch(() => {
