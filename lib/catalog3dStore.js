...
function adminCandidates() {
  try { return getSupabaseAdminCandidates(); } catch { return []; }
}

async function tableReadyFor(admin) {
  try {
    const { error } = await admin.from('catalog_3d_media').select('item_id,task_id', { count: 'exact', head: true });
    return !error;
  } catch { return false; }
}

async function firstReadableTableClient() {
  for (const admin of adminCandidates()) if (await tableReadyFor(admin)) return admin;
  return null;
}

async function tableReady() { return Boolean(await firstReadableTableClient()); }

function legacyTablePayload(payload = {}) {
  const { source_image_urls: _sourceImageUrls, model_storage_path: _modelStoragePath, ...legacy } = payload;
  return legacy;
}

async function writeTableRow(admin, payload) {
  try {
    const full = await admin.from('catalog_3d_media').upsert(payload, { onConflict: 'item_id' }).select('*').single();
    if (!full.error && full.data) return normalizeRow(full.data);
    const legacy = await admin.from('catalog_3d_media').upsert(legacyTablePayload(payload), { onConflict: 'item_id' }).select('*').single();
...
async function discoverStorageClient() {
  for (const supabase of adminCandidates()) if (await storageReadyFor(supabase)) return supabase;
  return null;
}
...
async function writeStorageRow(itemId, patch = {}) {
  ...
  for (const supabase of adminCandidates()) {
    ...
    try {
      const { error } = await supabase.storage.from(SYSTEM_BUCKET).upload(metadataPath(itemId), body, { contentType: 'application/json', cacheControl: '0', upsert: true });
      if (error) continue;
...
async function listStorageRows() {
  ...
  try {
    ...
    if (error) return [];
    const rows = await Promise.all((data || []).filter(file => file.name?.endsWith('.json')).map(async file => readStorageRow(decodeURIComponent(file.name.replace(/\.json$/i, '')))));
...
export async function catalog3DStoreReady() { return (await catalog3DStoreHealth()).ready; }
...
async function uploadModelBytes(itemId, bytes, contentType = 'model/gltf-binary') {
  if (!itemId || !bytes) return null;
  const path = `models/${encodeURIComponent(String(itemId))}.glb`;
  for (const supabase of adminCandidates()) {
    if (!(await storageReadyFor(supabase))) continue;
    try {
      const { error } = await supabase.storage.from(SYSTEM_BUCKET).upload(path, bytes, { contentType, cacheControl: '31536000', upsert: true });
      if (error) continue;
      storageReadyPromise = Promise.resolve(supabase);
      return path;
    } catch {}
...
export async function persistModelBytes(itemId, bytes, contentType = 'model/gltf-binary') {
  return uploadModelBytes(itemId, bytes, contentType);
}
