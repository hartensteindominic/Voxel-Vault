...
export function savePropertyDraft(draft) {
  ...
  const ids = [next.id, ...(Array.isArray(currentIds) ? currentIds : []).filter((id) => id !== next.id)].slice(0, MAX_DRAFTS);
  window.localStorage.setItem(INDEX_KEY, JSON.stringify(ids));
  for (let index = MAX_DRAFTS; index < (Array.isArray(currentIds) ? currentIds.length : 0); index += 1) {
    const stale = currentIds[index];
    if (stale && !ids.includes(stale)) window.localStorage.removeItem(propertyDraftStorageKey(stale));
  }
  return next;
}
