...
async function ensureBase(provider) {
  ...
  if (chainId?.toLowerCase() !== VOXELFLIP_CHAIN_ID.toLowerCase()) {
    throw new Error(`Please switch your wallet to ${VOXELFLIP_CHAIN_NAME}.`);
  }
  return chainId;
}

export function voxelflipConfigured() {
  return /^0x[a-fA-F0-9]{40}$/.test(VOXELFLIP_NFT_ADDRESS);
}
...
export async function mintVoxelFlip({ metadataUrl, voucherId, signature }) {
  if (!voxelflipConfigured()) throw new Error('VoxelFlip minting is not configured on this deployment yet.');
  ...
  const contract = new Contract(VOXELFLIP_NFT_ADDRESS, ABI, signer);
  ...
  return {
    ...
    explorerUrl: `${VOXELFLIP_EXPLORER_URL}/tx/${receipt?.hash || tx.hash}`,
    openSeaUrl: tokenId ? openSeaAssetUrl(tokenId) : '',
  };
}

export function openSeaAssetUrl(tokenId) {
  if (!VOXELFLIP_NFT_ADDRESS || tokenId == null) return '';
  const id = String(tokenId);
  const chain = VOXELFLIP_CHAIN_ID.toLowerCase();
  if (chain === '0x14a34') return `https://testnets.opensea.io/assets/base_sepolia/${VOXELFLIP_NFT_ADDRESS}/${id}`;
  if (chain === '0xaa36a7') return `https://testnets.opensea.io/assets/sepolia/${VOXELFLIP_NFT_ADDRESS}/${id}`;
  if (chain === '0x1') return `https://opensea.io/assets/ethereum/${VOXELFLIP_NFT_ADDRESS}/${id}`;
  return `https://opensea.io/assets/base/${VOXELFLIP_NFT_ADDRESS}/${id}`;
}
