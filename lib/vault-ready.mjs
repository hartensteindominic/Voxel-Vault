...
export function getVaultReadyReport(product = {}) {
  ...
  const requiredText = [
    ...
    ['mint_confirmed_at', 'server-verified mint receipt'],
    ['mint_owner', 'verified token owner'],
  ];
  ...
  if (!positiveInteger(product.mint_confirmed_block)) missing.push('confirmed mint block');
  if (text(product.mint_owner) && !EVM_ADDRESS.test(text(product.mint_owner))) invalid.push('verified token owner');
