...
function chooseDiverseViews(photos, maxViews = 4) {
  // For a visual-first property workflow, prefer the newest nearby frame first.
  // Distance breaks ties so a much farther image does not outrank an equally recent closer view.
  const sorted = [...photos].sort((a, b) => {
    const dateDelta = dateScore(b.shotDate) - dateScore(a.shotDate);
...
export async function fetchOpenStreetImagery(input = {}, options = {}) {
  ...
  try {
    ...
    return {
      ...
      selectionStrategy: 'newest-nearby-first',
      license: KARTAVIEW_LICENSE,
      attribution: 'KartaView contributors',
      termsUrl: KARTAVIEW_TERMS_URL,
      ...
      note: photos.length
        ? 'Newest nearby open street imagery is shown first. Visual proximity does not prove that every frame depicts the selected parcel, so compare the facade before generation.'
