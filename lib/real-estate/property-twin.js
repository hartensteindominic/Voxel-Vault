...
export function normalizePropertyTwin(input = {}) {
  ...
  const physicalChecks = {
    buildingFootprint: validPolygonGeometry(structure.buildingGeometry),
    buildingHeight: structure.heightMeters !== null && structure.heightMeters > 0,
    authoritativeSource: validLineageSource(structure.source),
  };
  ...
  const physicalState = stateFromChecks(physicalChecks);
  ...
  const verifiedSpatialTwin = geographyState === PROPERTY_TRUTH_STATES.VERIFIED && physicalState === PROPERTY_TRUTH_STATES.VERIFIED;
