...
export function buildPropertySliceSandbox(input = {}) {
  ...
  const hypotheticalFraction = amountCents / propertyReferencePriceCents;
  ...
  const benchmarkEquivalentCents = Math.max(1, Math.round(amountCents * relativeSliceWeight));
  ...
  return {
    ...
    note: 'The percentage is a mathematical comparison only. It does not represent legal ownership, equity, rent rights, title, a security, or a claim against the property.',
  };
}
