...
export function buildSandboxPropertyPurchase(input = {}) {
  ...
  const requestedDemoUsdCents = cents(
  ...
  // Demo USD is a sandbox-only teaching balance, not customer funds. It should never dead-end
  // the product tour. If a scaled test slice costs more than the remaining demo balance, top up
  // exactly enough fake USD to let the test continue. This has no live-money effect.
  ...
  return {
    ...
    balances: {
      demoUsdRequestedBeforeCents: requestedDemoUsdCents,
      demoUsdAutoTopUpCents,
      demoUsdBeforeCents,
      demoUsdAfterCents,
    },
    legalEffects: {
