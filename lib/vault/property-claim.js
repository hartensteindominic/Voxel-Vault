...
export const PROPERTY_REVIEW_RULES = Object.freeze({
  reviewerNoteMinLength: 20,
  humanApprovalRequired: true,
  approvalCreatesDeed: false,
  approvalMintsPassport: false,
  approvalSetsOnchainRegistryVerified: false,
});
...
export function canonicalizePropertyIdentity(input = {}) {
  ...
  if (countryCode === 'US' && !subdivisionCode) throw new Error('A state/subdivision code is required for U.S. property claims.');
  if (subdivisionCode.length > 32) throw new Error('The state/subdivision code is too long.');
  if (!countyCode) throw new Error('An assessor jurisdiction/county code is required.');
  if (countyCode.length > 64) throw new Error('The assessor jurisdiction/county code is too long.');
  if (!parcelId) throw new Error('An assessor/parcel identifier is required.');
  if (parcelId.length > 128) throw new Error('The assessor/parcel identifier is too long.');

  // Address text is intentionally excluded. Postal formatting and street aliases are
  // not stable enough to enforce the one-canonical-twin rule. The assessor jurisdiction
  // plus normalized parcel identifier is the canonical identity input.
...
export function evaluatePropertyClaim(input = {}) {
  ...
  // Exact boolean only. Values such as the string "false" must never satisfy the
  // authorization gate when the API is called outside the first-party UI.
  ...
  return {
    ...
    note: canEnterOfficialReview
      ...
      : 'Official parcel linkage requires the owner or an authorized controller. Unverified building art remains a separate collectible path.',
  };
}
