...
export function buildVaultManifest({
  ...
  const scope = clean(providerAccountScope).toLowerCase();
  const reitEntries = (Array.isArray(reitPositions) ? reitPositions : [])
    ...
    .map((position, index) => ({
      ...
      status: scope === 'user-bound' ? 'provider-reported-user-bound' : 'provider-reported',
      truthLabel: scope === 'user-bound' ? 'USER-BOUND PROVIDER POSITION' : 'PROVIDER POSITION REPORTED',
      sourceLabel: `${clean(provider || 'PROVIDER').toUpperCase()} ${clean(providerEnvironment || 'sandbox').toUpperCase()} ACCOUNT${scope === 'user-bound' ? ' · USER BOUND' : ''}`,
      href: '/real-estate/reits',
      note: scope === 'user-bound'
        ? 'This security position was returned by the provider account bound to this Voxel Vault identity. It is not a deed or direct ownership of a specific property.'
        : 'This is a security position reported by the configured provider account. It is not a deed or direct ownership of a specific property.',
    }));
