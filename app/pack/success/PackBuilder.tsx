...
function zipBytes(files:{name:string;data:Uint8Array}[]){
  ...
  for(const file of files){
    ...
    lv.setUint32(0,0x04034b50,true);lv.setUint16(4,20,true);lv.setUint16(6,0x0800,true);lv.setUint16(8,0,true);lv.setUint16(10,time,true);lv.setUint16(12,date,true);lv.setUint32(14,crc,true);lv.setUint32(18,file.data.length,true);lv.setUint32(22,file.data.length,true);lv.setUint16(26,name.length,true);lv.setUint16(28,0,true);
    local.set(name,30);local.set(file.data,30+name.length);locals.push(local);

    const central=new Uint8Array(46+name.length);
    const cv=new DataView(central.buffer);
    cv.setUint32(0,0x02014b50,true);cv.setUint16(4,20,true);cv.setUint16(6,20,true);cv.setUint16(8,0x0800,true);cv.setUint16(10,0,true);cv.setUint16(12,time,true);cv.setUint16(14,date,true);cv.setUint32(16,crc,true);cv.setUint32(20,file.data.length,true);cv.setUint32(24,file.data.length,true);cv.setUint16(28,name.length,true);cv.setUint16(30,0,true);cv.setUint16(32,0,true);cv.setUint16(34,0,true);cv.setUint16(36,0,true);cv.setUint32(38,0,true);cv.setUint32(42,offset,true);central.set(name,46);centrals.push(central);
    offset+=local.length;
  }
...
export default function PackBuilder({sessionId}:{sessionId:string}){
  ...
  useEffect(()=>{
    try{
      const stored=sessionStorage.getItem('voxelPackBrief');
      if(stored) setBrief({...defaultBrief,...JSON.parse(stored)});
    }catch{}
  },[]);

  async function chooseReference(event:React.ChangeEvent<HTMLInputElement>){
  ...
  async function generate(){
    ...
    try{
      ...
      const separated=await splitSheet(data.image,data.names||[]);
  ...
  async function downloadZip(){
    if(!sheet||assets.length!==25)return;
    setMessage('Packaging your ZIP…');
    const encoder=new TextEncoder();
    ...
    saveBlob(zipBytes(files),'voxel-vault-custom-25-asset-pack.zip');
    setMessage('ZIP downloaded. Keep it somewhere safe.');
  }

  return <main className={styles.page}>
    <nav className={styles.nav}><a href="/studio"><span>VV</span><b>Voxel Vault</b></a><em>PAYMENT COMPLETE</em></nav>
    <section className={styles.shell}>
      ...
      {status!=='done'&&<div className={styles.builder}>
        ...
        <div className={styles.styles}><b>Finish</b><div>{[['polished','Polished'],['chunky','Chunky'],['cute','Cute'],['dark','Dark fantasy']].map(([value,label])=><button key={value} className={brief.style===value?styles.active:''} onClick={()=>setBrief(v=>({...v,style:value}))}>{label}</button>)}</div></div>
        ...
        <button className={styles.generate} disabled={status==='generating'} onClick={generate}>{status==='generating'?'Generating your 25 assets…':'Generate my pack'}</button>
        <small className={styles.note}>Generation can take around a minute. Keep this tab open.</small>
      </div>}
      ...
      <footer><a href="/studio">← Back to Voxel Vault</a><span>AI-generated output can vary. Inspect assets before production use.</span></footer>
    </section>
  </main>;
}
