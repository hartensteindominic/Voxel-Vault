...
const styles = {
  nav: {
    position: 'fixed', zIndex: 90, left: '50%', bottom: 'max(8px, env(safe-area-inset-bottom))', transform: 'translateX(-50%)',
    width: 'min(590px, calc(100vw - 16px))', display: 'grid', gap: 5, padding: 7, boxSizing: 'border-box',
    border: '1px solid rgba(84,64,75,.12)', borderRadius: 25, background: 'rgba(255,252,246,.96)',
  ...
  item: {
    minWidth: 0, minHeight: 54, display: 'grid', placeItems: 'center', alignContent: 'center', gap: 3, padding: '5px 3px',
    borderRadius: 18, color: '#4b424c', textDecoration: 'none', border: '1px solid transparent', touchAction: 'manipulation',
    transition: 'transform .12s ease, background .12s ease, box-shadow .12s ease',
  },
  itemActive: {
    border: '1px solid rgba(49,145,113,.22)', background: 'linear-gradient(180deg,#efffd5,#dcf7cb)',
    boxShadow: '0 4px 0 #b7db87, 0 8px 20px rgba(77,125,60,.12)', transform: 'translateY(-1px)',
  },
  icon: {
    minWidth: 30, height: 28, padding: '0 7px', borderRadius: 10, display: 'grid', placeItems: 'center', fontSize: 12,
    fontWeight: 1000, letterSpacing: '-.04em', color: '#776d78', background: 'rgba(112,87,119,.07)',
  },
