...
function BuyBoth({item}){
  ...
  return <div className="vv3-buyBoth">
    {ready
      ...
      : <div className="vv3-buyButton vv3-buyButtonDisabled">CONCEPT — NOT FOR SALE</div>}
    <div className="vv3-buyPerks"><span>Physical reference</span><span>Image-based preview</span><span>Not minted</span></div>
    {item.customerPriceUsd&&<div className="vv3-priceNote">Reference price only · checkout requires a verified supplier, licensed 3D model, and pre-minted token</div>}
  </div>;
}
...
export default function RealWorldCommerceHome(){
  ...
  return <main className="vv3-home">
    ...
    <header className="vv3-header"><div className="vv3-topbar"><Brand/><nav className="vv3-desktopNav" aria-label="Primary navigation"><Link href="/discover">Discover</Link><Link href="/marketplace">Marketplace</Link><Link href="/room">My vault</Link><Link href="/ai">Intelligence</Link></nav><Link className="vv3-headerCta" href="#collection">View concepts <Icon name="arrow" size={15}/></Link></div></header>
    <section className="vv3-hero">
      ...
      <div className="vv3-heroCopy">
        ...
        <div className="vv3-heroActions"><Link className="vv3-primaryCta" href="#collection">View concepts <Icon name="arrow" size={17}/></Link><Link className="vv3-textCta" href="/room"><span><Icon name="cube" size={14}/></span> Open Vault</Link></div>
        <div className="vv3-proofRow"><span><Icon name="shield" size={16}/><b>Product source recorded</b></span><span><Icon name="cube" size={16}/><b>Licensed 3D required</b></span><span><Icon name="shield" size={16}/><b>Pre-mint required</b></span></div>
      </div>
    ...
    <section className="vv3-collection" id="collection">
      <div className="vv3-collectionHead"><div><div className="vv3-sectionLabel"><span>01</span> CONCEPT GALLERY</div><h2>Inspect the concept.<br/><em>Publish only when verified.</em></h2></div><p>These are reference concepts, not purchasable bundles. A listing can enter the store only after its supplier route, licensed GLB/GLTF, model rights, inventory, shipping, and pre-minted token all pass the Vault Ready checks.</p></div>
