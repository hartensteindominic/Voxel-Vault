...
function ProductTile({ item, index }) {
  ...
  return <article className="vs-card">
    ...
    <div className="vs-cardBody"><div><small>{state === 'verified' ? 'COLLECTIBLE APPROVED' : state === 'review' ? 'INTERACTIVE PREVIEW READY' : 'PREVIEW PREBUILDING'}</small><h3>{item.name}</h3></div><strong>${item.customerPriceUsd}</strong></div>
    <div className="vs-cardSignals"><span className="is-source">PRODUCT SYNCED</span><span>{state === 'verified' ? 'MATCH APPROVED' : state === 'review' ? 'PREVIEW READY' : 'PREBUILDING'}</span><span>{ready ? 'BUY READY' : 'COMING SOON'}</span></div>
...
export default function VoxelStorefront() {
  ...
  return <main className="vs-page">
    ...
    <section className="vs-featured"><div className="vs-sectionHead"><div><small>ENTERING THE VAULT</small><h2>Objects worth keeping.</h2><p>Products can appear while their interactive collectible is still prebuilding. Buying opens only when the product, fulfillment path, and collectible match are ready.</p></div><Link href="/marketplace">View all {REAL_WORLD_CATALOG.length} products ↗</Link></div><div className="vs-grid">{picks.map((item, index) => <ProductTile key={item.id} item={item} index={index}/>)}</div></section>
    ...
    <section className="vs-launch"><div><small>QUALITY FIRST</small><h2>{heroReady ? 'The featured object is ready.' : 'The catalog can grow while collectibles finish.'}</h2><p>Generated previews are never treated as exact automatically. If a model does not closely match the physical item, it stays unpublished as the official collectible and buying remains closed for that product.</p></div><Link href="/marketplace">Explore product readiness ↗</Link></section>
