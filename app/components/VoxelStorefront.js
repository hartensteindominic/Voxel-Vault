...
function ProductTile({ item, index }) {
  ...
  return <article className="vs-card">
    ...
    <div className="vs-cardBody"><div><small>{state === 'verified' ? 'VERIFIED 3D COLLECTIBLE' : state === 'review' ? '3D UNDER REVIEW' : 'EXACT 3D PENDING'}</small><h3>{item.name}</h3></div><strong>${item.customerPriceUsd}</strong></div>
    ...
    <div className="vs-cardFoot"><span>PHYSICAL + 3D COLLECTIBLE</span><Link href={`/marketplace?object=${encodeURIComponent(item.id)}`}>Inspect object ↗</Link></div>
  </article>;
}
...
export default function VoxelStorefront() {
  ...
  return <main className="vs-page">
    ...
    <section className="vs-hero">
      <div className="vs-heroCopy"><div className="vs-eyebrow"><i/> PHYSICAL OBJECTS · VERIFIED 3D COLLECTIBLES</div><h1>Find it real.<br/><em>Keep it digital.</em></h1><p>Voxel Vault turns real CJ-sourced products into collectible objects with persistent digital identity. Shop in USD, receive the physical product at home, and keep its matching verified 3D collectible after the commerce and chain checks are complete.</p><div className="vs-heroActions"><Link href="/marketplace" className="vs-primary">Enter the object index <span>↗</span></Link><a href="#how" className="vs-secondary">See the loop</a></div><div className="vs-assurance"><span>Card payment</span><span>Home delivery</span><span>No wallet required to buy</span><span>3D collectible included</span></div><div className="vs-pulse" aria-label="Catalog readiness"><div><strong>{REAL_WORLD_CATALOG.length}</strong><span>CJ objects mapped</span></div><div><strong>{verifiedCount}</strong><span>Exact 3D verified</span></div><div><strong>{readyCount}</strong><span>Checkout ready</span></div></div></div>
    ...
    <footer className="vs-footer"><span>VOXEL VAULT</span><p>Real object. Verified 3D. One history.</p><div><Link href="/terms">Terms</Link><Link href="/privacy">Privacy</Link></div></footer>
  </main>;
}
