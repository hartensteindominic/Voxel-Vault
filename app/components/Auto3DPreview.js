...
const POLL_MS = 7000;
const cacheKey=(kind,id)=>`voxel:${kind}:${id}`;
...
export default function Auto3DPreview({ item, hero = false }) {
  ...
  useEffect(()=>{
    ...
    const modelKey=cacheKey('model',item.id);const taskKey=cacheKey('meshy-task',item.id);
  ...
  return <div className="vv3-generationState"><div className="vv3-generationOrb">3D</div><strong>{status==='error'?'3D preview unavailable':'Building interactive 3D preview'}</strong><small>{status==='error'?error:`Meshy is generating this object from the CJ product image${progress?` · ${progress}%`:''}. You can keep browsing while it finishes.`}</small>{status!=='error'&&<div className="vv3-progress"><i style={{width:`${Math.max(6,Math.min(100,progress||8))}%`}}/></div>}</div>;
}
