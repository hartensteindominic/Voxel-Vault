...
type Payload={asset?:{name?:string;dataUrl?:string};mesh?:{status?:string;progress?:number;taskId?:string;modelUrl?:string;error?:string};generationsLeft?:number;idea?:string;updatedAt?:string};
...
export default function MintTradePage(){
 ...
 const [voxel,setVoxel]=useState<Payload|null>(null);
 const [accountUser,setAccountUser]=useState<any>(null);
 const [wallet,setWallet]=useState('');
 const [stage,setStage]=useState<'loading'|'ready'|'connecting'|'preparing'|'minting'|'verifying'|'done'|'error'>('loading');
  ...
  if(voxelAccountConfigured()){
   ...
   supabase.auth.getSession().then(async({data})=>{
    ...
    try{
     ...
     if(!local?.asset?.dataUrl&&record?.payload?.asset?.dataUrl){setVoxel(record.payload as Payload);setStage('ready');setMessage(record.payload.mesh?.status==='ready'?'Restored from your Google account. Ready to mint.':'Restored from your Google account. Finish its 3D mesh first.');}
    }catch{}
   });
  }
 ...
 async function recoverTask(current:Payload){
 ...
 async function connect(){
  ...
  catch(error:any){if(error?.code==='NO_WALLET_PROVIDER'&&error?.deepLink){location.href=error.deepLink;return ''}setStage('error');setMessage(error instanceof Error?error.message:'Wallet connection failed.');return ''}
 }
 ...
 async function mint(){
  ...
  }catch(error:any){if(error?.code==='NO_WALLET_PROVIDER'&&error?.deepLink){location.href=error.deepLink;return}setStage('error');setMessage(error instanceof Error?error.message:'VoxelFlip minting failed.')}
 }
