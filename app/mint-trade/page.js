...
export default function MintTradePage(){
 ...
 async function runScanner(){
  ...
  try{const query=new URLSearchParams({wallet});if(minted?.tokenId)query.set('tokenId',minted.tokenId);const response=await fetch(`/api/voxelflip/trader?${query}`,{cache:'no-store'});const data=await response.json();if(response.ok)setScanner(data);else setScanner({...data,scanner:'error'})}catch{setScanner({scanner:'error',marketDataConfigured:false,executionFoundationReady:false,automaticSigningActive:false})}finally{setScannerBusy(false)}
 }
 ...
 const executionLabel=scanner?.automaticSigningActive?'AUTOPILOT ON':scanner?.executionFoundationReady?'AUTOPILOT READY':'AUTOPILOT SETUP';
 ...
 return <main className={styles.page}>
  ...
  <header className={styles.hero}><p>VOXELPOP → VOXELFLIP → MARKET</p><h1>Mint it.<br/><em>Then watch the market.</em></h1><span>Your 3D voxel stays yours. VoxelFlip recovers completed mints and continuously monitors the market around your connected wallet.</span></header>
  ...
  <section className={styles.bot}>
   ...
   <div className={styles.mode}><div><b>Market scanning</b><span>{wallet?(scanner?.scanner==='live'?'LIVE · refreshes every 30 sec':scanner?.scanner==='configuration-needed'?'OPENSEA KEY NEEDED':'CONNECTING…'):'CONNECT WALLET'}</span></div><div><b>Trade execution</b><span>{executionLabel}</span></div><div><b>Collection</b><span>VoxelFlip · Base</span></div></div>
   ...
   <div className={styles.stats}><article><small>ACTIVE LISTINGS</small><b>{scanner?.listings??'—'}</b></article><article><small>OFFERS RECEIVED</small><b>{scanner?.offersReceived??'—'}</b></article><article><small>PORTFOLIO</small><b>{scanner?.portfolio?'LIVE':'—'}</b></article><article><small>EXECUTION</small><b>{scanner?.automaticSigningActive?'AUTO':scanner?.executionFoundationReady?'READY':'SETUP'}</b></article></div>
   <div className={styles.mode}><div><b>Max per trade</b><span>{risk.maxTradeEth??'—'} ETH</span></div><div><b>Daily spend cap</b><span>{risk.maxDailySpendEth??'—'} ETH</span></div><div><b>Daily loss stop</b><span>{risk.maxDailyLossEth??'—'} ETH</span></div></div>
   ...
   <div className={styles.automation}><div><span>✓</span><p><b>Watch offers, listings and portfolio automatically</b><small>Refresh market state every 30 seconds while the workspace is open.</small></p></div><div><span>⚡</span><p><b>No-prompt execution uses a separate bounded signer</b><small>A personal MetaMask wallet is not silently bypassed. The owner and mint-signer keys stay separate from trading.</small></p></div><div><span>■</span><p><b>Hard trading limits stay visible</b><small>Default max inventory: {risk.maxInventory??'—'} · minimum modeled edge: {risk.minimumEdgeBps??'—'} bps.</small></p></div><div><span>↻</span><p><b>Recovery before re-mint</b><small>Consumed vouchers are recovered before a new mint can ever be created.</small></p></div></div>
