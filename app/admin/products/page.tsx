    ...
    <div className="workspace">
      ...
      <section className="editor">
        {!selected?<div className="empty">Create a supplier draft to begin.</div>:<>
          ...
          <div className="formGrid">{fields.map(([key,label])=><label className={key.endsWith('_url')||key.endsWith('_uri')||key==='model_license'||key==='mint_tx_hash'?'wide':''} key={key}>{label}<input type={key.includes('price')?'number':'text'} value={selected[key]??''} onChange={(event)=>change(key,event.target.value)}/></label>)}</div>
          <div className="states">
            ...
            <label>Pre-mint<select value={selected.mint_status} onChange={(e)=>change('mint_status',e.target.value)}><option value="unverified">Unverified</option><option value="pending">Pending</option><option value="confirmed">Confirmed</option><option value="failed">Failed</option></select></label>
