...
import { bindIncreaseSandboxAccount, getProviderAccountBinding, publicBindingSummary } from '../../../../../../lib/banking/provider-account-binding.js';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return response({ ok: true, authorized: true, ...onboarding, entity, binding: publicBindingSummary(bindingState.binding), bindingSetupRequired: bindingState.setupRequired, bindingError: bindingState.error || '', note: 'Owner-only Increase sandbox onboarding status. Provider identity is scoped to the signed-in owner through a server-written binding; no customer identity fields or account-number details are returned.' });
  } catch (error: any) {
...
export async function POST(request: Request) {
  ...
  try {
    if (action === 'create_session') {
      ...
      const result = await createIncreaseSandboxOnboardingSession({ programId: requestedProgramId, entityId: String(body?.entityId || ''), redirectUrl: redirectUrl.toString() }, process.env);
    ...
    if (action === 'simulate_submit') {
      ...
      return response({ ok: true, authorized: true, action, ...result, note: 'Sandbox-only simulated form submission. This creates test data and is not customer KYC approval.' });
    }
    if (action === 'simulate_valid') {
      const entity = await simulateIncreaseSandboxEntityValid(String(body?.entityId || ''), process.env);
      return response({ ok: true, authorized: true, action, entity, canMoveRealMoney: false, note: 'Sandbox-only validation simulation. This is not a real KYC/CIP/AML decision.' });
    }
    if (action === 'bootstrap') {
      const result = await bootstrapIncreaseSandboxAccount({ entityId: String(body?.entityId || ''), programId: String(body?.programId || ''), accountName: String(body?.accountName || '') }, process.env);
      const binding = await bindOwnerSandboxAccount(auth, result, 'increase-sandbox-bootstrap');
      return response({ ok: true, authorized: true, action, ...result, binding, note: 'Sandbox Account ownership is bound server-side to the signed-in owner. Provider validation is sandbox simulation only; no real money can move.' });
    }
    if (action === 'complete_setup') {
      const result = await completeIncreaseSandboxSetup({ entityId: String(body?.entityId || ''), programId: String(body?.programId || ''), accountName: String(body?.accountName || '') }, process.env);
      const binding = await bindOwnerSandboxAccount(auth, result, 'increase-hosted-sandbox-onboarding');
      return response({ ok: true, authorized: true, action, ...result, binding, note: 'Sandbox-only setup: validation was simulated before creating/reusing a test Account and Account Number, then the provider Account was bound server-side to the signed-in owner. This is not real KYC approval and no real money can move.' });
    }
    return response({ ok: false, authorized: true, canMoveRealMoney: false, error: 'Unsupported Increase sandbox onboarding action.' }, 400);
  } catch (error: any) {
    return providerError(error);
  }
}
