...
export async function POST(request: Request) {
  ...
  if (auth.ok === false) return NextResponse.json({ error: 'Sign in with Google before creating or buying a VoxelPop asset.' }, { status: auth.status });
  ...
  try {
    ...
    await recordVoxelPopEvent({
      ...
      attribution,
      details: { amount_cents: 199, currency: 'usd', account_linked: true },
    });

    return NextResponse.json({ url: session.url, accountLinked: true });
  } catch (error) {
