...
export async function GET(request:Request){
  ...
  try{
    const url=new URL(request.url);const sessionId=url.searchParams.get('sessionId')||'';const taskId=url.searchParams.get('taskId')||'';const session=await paidSession(sessionId);if(!session) return NextResponse.json({error:'A completed VoxelPop 3D Asset purchase is required.'},{status:403});
    if(!taskId||session.metadata?.[taskKey]!==taskId) return NextResponse.json({error:'This 3D task does not belong to the current purchase.'},{status:403});
    ...
    const status=String(data?.status||'PENDING');const modelUrl=data?.model_urls?.glb||null;if(url.searchParams.get('download')==='1'){if(!modelUrl) return NextResponse.json({error:'The GLB file is not ready yet.'},{status:409});return NextResponse.redirect(modelUrl);}
    return NextResponse.json({configured:true,status,progress:modelUrl?100:Number(data?.progress||0),modelUrl,thumbnailUrl:data?.alpha_thumbnail_url||data?.thumbnail_url||null,thumbnailUrls:data?.thumbnail_urls||null,error:data?.task_error?.message||null});
