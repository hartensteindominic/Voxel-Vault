...
export async function POST(request:Request){
 ...
 try{
  ...
  if(existingTask){if(!forceRestart){await recordVoxelPopEvent({eventName:'mesh_started',eventKey:`mesh_started:${sessionId}:${existingTask}`,flowId,stripeSessionId:sessionId,attribution,details:{reused:true}});return NextResponse.json({configured:true,reused:true,taskId:existingTask})}const existingResponse=await fetch(`${MESH_ENDPOINT}/${encodeURIComponent(existingTask)}`,{headers:{Authorization:`Bearer ${apiKey}`},cache:'no-store'});const existingData=await existingResponse.json().catch(()=>({}));const existingStatus=String(existingData?.status||'').toUpperCase();if(!existingResponse.ok||!['FAILED','EXPIRED','CANCELED','CANCELLED'].includes(existingStatus)){await recordVoxelPopEvent({eventName:'mesh_started',eventKey:`mesh_started:${sessionId}:${existingTask}`,flowId,stripeSessionId:sessionId,attribution,details:{reused:true}});return NextResponse.json({configured:true,reused:true,taskId:existingTask})}if(retryCount>=1)return NextResponse.json({error:'This mesh has already used its retry. Please contact support if it still cannot finish.'},{status:409});retryCount+=1;}
  const meshPayload={image_url:image,model_type:'smart-topology',ai_model:'meshy-t2',image_enhancement:true,target_polycount:12000,should_texture:true,enable_pbr:true,texture_resolution:'2k',target_formats:['glb']};
