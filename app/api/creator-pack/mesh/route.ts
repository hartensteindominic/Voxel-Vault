...
export async function POST(request:Request){
 ...
 try{
  ...
  if(existingTask){if(!forceRestart)return NextResponse.json({configured:true,reused:true,taskId:existingTask});const existingResponse=await fetch(`${MESH_ENDPOINT}/${encodeURIComponent(existingTask)}`,{headers:{Authorization:`Bearer ${apiKey}`},cache:'no-store'});const existingData=await existingResponse.json().catch(()=>({}));const existingStatus=String(existingData?.status||'').toUpperCase();if(!existingResponse.ok||!['FAILED','EXPIRED','CANCELED','CANCELLED'].includes(existingStatus))return NextResponse.json({configured:true,reused:true,taskId:existingTask});if(retryCount>=1)return NextResponse.json({error:'This mesh has already used its retry. Please contact support if it still cannot finish.'},{status:409});retryCount+=1;}
  ...
  await stripe.checkout.sessions.update(sessionId,{metadata:{...(session.metadata||{}),[taskKey]:taskId,[retryKey]:String(retryCount),mesh_name_0:String(body?.name||'your-voxel').slice(0,80),mesh_idea_0:idea.slice(0,120)}});return NextResponse.json({configured:true,reused:false,taskId});
...
export async function GET(request:Request){
 ...
 try{
  ...
  const status=String(data?.status||'PENDING');const modelUrl=typeof data?.model_urls?.glb==='string'?data.model_urls.glb:'';
  ...
  if(url.searchParams.get('download')==='1'){
   ...
   const modelResponse=await fetch(modelUrl,{cache:'no-store'});if(!modelResponse.ok)return NextResponse.json({error:'The generated GLB could not be downloaded.'},{status:502});
