...
export async function POST(request: Request) {
  try {
    ...
    if (receipt.to?.toLowerCase() !== contractAddress.toLowerCase()) return NextResponse.json({ error: 'The transaction did not mint from the VoxelFlip contract.' }, { status: 409 });
    ...
    await recordVoxelPopEvent({
      ...
      details: { tokenId, wallet: wallet.toLowerCase(), chain: process.env.NEXT_PUBLIC_VOXELFLIP_CHAIN_NAME || 'Base', payment_method: entitlement.paymentMethod },
    });

    return NextResponse.json({ confirmed: true, tokenId, wallet, openSeaUrl: openSeaUrl(contractAddress, tokenId), explorerUrl: `${process.env.NEXT_PUBLIC_VOXELFLIP_EXPLORER_URL || 'https://basescan.org'}/tx/${txHash}` });
  } catch (error) {
