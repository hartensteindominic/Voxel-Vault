...
async function refreshPersistedModel(apiKey: string, saved: any, clientTaskId: string | null = null) {
  ...
  if (!providerTaskId) {
    return { providerRefreshed: false, ...publicState(saved, await displayUrlFor(saved)) };
  }
  ...
  if (!response.ok) {
    // A locally persisted GLB remains usable even if Meshy's old job-status URL
    // has aged out. Generate a fresh private Storage URL instead of failing.
    const cachedDisplay = saved?.model_storage_path ? await displayUrlFor(saved) : null;
    if (cachedDisplay) {
      return {
        providerRefreshed: false,
        cachedBinaryFallback: true,
        providerRefreshFailed: true,
        ...publicState(saved, cachedDisplay),
      };
    }
    throw new Error(providerErrorMessage(data, 'The saved 3D link expired before Voxel Vault could refresh it. Try the build again from your photo.'));
  }
  ...
  const patch = {
    ...
    model_url: providerModelUrl || (modelStoragePath ? saved?.model_url : null),
  ...
  const updated = saved?.item_id ? (await saveCatalog3D(saved.item_id, patch) || { ...saved, ...patch }) : { ...saved, ...patch };
  const finalRow = {
    ...updated,
...
export async function GET(request: Request) {
  ...
  try {
    if ((atlasIdRaw || draftIdRaw) && !taskId) {
      ...
      if (apiKey && (saved.model_url || saved.model_storage_path)) {
        try {
          ...
          const cachedDisplay = saved.model_storage_path ? await displayUrlFor(saved) : null;
          if (cachedDisplay) return privateJson({ ok: true, exists: true, cachedResumeAvailable: true, cachedBinaryFallback: true, ...publicState(saved, cachedDisplay) });
        }
      }
    ...
    if (!response.ok) {
      ...
      return privateJson({ ok: false, error: providerErrorMessage(data, 'Could not refresh the property 3D model. Try the build again from your photo.') }, { status: response.status });
    }
    ...
    const finalRow = {
      ...
      model_url: providerModelUrl || (modelStoragePath ? updated?.model_url : null),
      thumbnail_url: thumbnailUrl || updated?.thumbnail_url || null,
      error: data?.task_error?.message || null,
    };
    return privateJson({ ok: true, exists: true, providerRefreshed: true, cachedBinaryReady: Boolean(modelStoragePath), ...publicState(finalRow, await displayUrlFor(finalRow)) });
  } catch (error) {
