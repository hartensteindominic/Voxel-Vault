...
export default function RealVoxelForgePage(){
  ...
  const selected=useMemo(()=>selectedIds.map(id=>assets.find(asset=>asset.tokenId===id)).filter(Boolean),[selectedIds,assets]);
  ...
  async function connect(){
    ...
    try{
      ...
      setStatus('Wallet connected. Step 1: scan your real VoxelFlip NFTs on Base. This scan is read-only.');
  ...
  async function scanAssets(){
    ...
    try{
      ...
      setStatus('Reading the reviewed production VoxelFlip collection and verifying which tokens this wallet owns. No transaction is being requested.');
      const q=new URLSearchParams({wallet});
      const response=await fetch(`/api/forge/owned-assets?${q}`,{cache:'no-store'});
      const data=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(data.error||'Could not scan your VoxelFlip NFTs.');
      const normalized=Array.isArray(data.nfts)?data.nfts.map(displayAsset):[];
      setAssets(normalized);setScanInfo(data);
      if(normalized.length>=3)setStatus(`Found ${normalized.length} wallet-owned VoxelFlip NFTs. Step 2: choose exactly 3.`);
      else setStatus(`Found ${normalized.length} wallet-owned VoxelFlip NFT${normalized.length===1?'':'s'}. You need 3 distinct VoxelFlips for a real 3→1 test.`);
  ...
  async function getSepoliaProvider(){
    ...
    const balance=await browserProvider.getBalance(wallet);
  ...
  async function verifyClone(){
    ...
    try{
      if(selected.length!==3)throw new Error('Select exactly 3 real VoxelFlips first.');
      ...
      setStatus(`Test Forge verified. Current 3→1 merge fee is ${formatEther(price)} Base Sepolia ETH. Step 3: review and import metadata copies of your 3 real VoxelFlips.`);
    }catch(e){setError(errorText(e));setStatus('')}
    finally{setBusy(false)}
  }
  ...
  async function importSelected(){
    ...
    try{
      if(!cloneVerified||selected.length!==3)throw new Error('Verify the test Forge with exactly 3 selected VoxelFlips first.');
      if(selected.some(item=>!String(item.tokenURI||'').trim()))throw new Error('One selected production NFT has no readable tokenURI, so it cannot be copied safely into the test Forge.');
      const browserProvider=await getSepoliaProvider();
      ...
      const iface=new Interface(CLONE_ABI);const tokenIds=[];
      ...
      setStatus(`Step 3 complete. Production VoxelFlips #${selected.map(item=>item.tokenId).join(', #')} remain untouched; test Common copies #${tokenIds.join(', #')} now exist inside your Sepolia Forge.`);
  ...
  async function forgeRare(){
    ...
    try{
      ...
      setStatus('Step 4A: MetaMask will ask you to SIGN the ForgeRequest. This signature alone does not spend ETH and cannot touch your production VoxelFlips.');
      ...
      const iface=new Interface(CLONE_ABI);let event=null;
      ...
      setRare(result);
      setStatus(`Complete. The 3 test Common copies were burned and Rare #${result.tokenId} was minted on Base Sepolia. Your original production VoxelFlips are still untouched on Base mainnet.`);
      const balance=await browserProvider.getBalance(wallet);setBalanceEth(formatEther(balance));
    }catch(e){setError(errorText(e));setStatus('')}
    finally{setBusy(false)}
  }
  ...
  return <main className={styles.page}>
    <nav className={styles.nav}>
      ...
      <em>REAL VOXELS → TEST FORGE</em>
    </nav>

    <div className={styles.shell}>
      <header className={styles.hero}>
        <p>GUIDED REAL-ASSET LINEAGE TEST</p>
        <h1>Pick 3 real voxels.<br/><em>Forge a Rare.</em></h1>
        <span>We read your real VoxelFlip NFTs on Base, copy only their metadata into your Base Sepolia Forge, then run the same 3→1 contract path you already proved. The production NFTs never leave your wallet.</span>
      </header>

      <div className={styles.steps}>
        {[
          ['1','SCAN'],['2','SELECT 3'],['3','IMPORT COPIES'],['4','SIGN + FORGE'],['5','RARE READY'],
        ].map(([number,label],index)=><div key={number} className={`${styles.step} ${stage===index?styles.active:''} ${stage>index?styles.done:''}`}><small>{stage>index?'✓':number}</small><b>{label}</b></div>)}
      </div>

      <section className={styles.panel}>
        <div className={styles.row}>
          <div><small>WALLET</small><b>{wallet?short(wallet):'Not connected'}</b>{balanceEth&&<span>{Number(balanceEth).toFixed(5)} Base Sepolia ETH</span>}</div>
          <button onClick={connect} disabled={busy}>{wallet?'RECONNECT':'CONNECT METAMASK'}</button>
        </div>

        <div className={styles.safety}>
          <b>Your Base mainnet NFTs are read-only here.</b>
          <span>The browser never calls approve, transferFrom, safeTransferFrom, burn, list, or sign against the production VoxelFlip contract. The only write contract used below is your Base Sepolia Forge clone.</span>
        </div>

        <div className={styles.sectionHead}><small>STEP 1 · READ REAL VOXELFLIPS</small><h2>See what this wallet actually owns.</h2><p>The server checks the reviewed production VoxelFlip contract, then independently verifies ownerOf on Base before an NFT can appear here.</p></div>
        <button className={styles.primary} onClick={wallet?scanAssets:connect} disabled={busy}>{busy?'CHECKING…':wallet?'SCAN MY REAL VOXELFLIPS':'CONNECT METAMASK'}</button>
        {scanInfo?.sourceWarning&&<div className={styles.notice}><b>SCAN NOTE</b><span>{scanInfo.sourceWarning}</span></div>}
        {scanInfo&&<div className={styles.notice}><b>PRODUCTION COLLECTION · READ ONLY</b><span>{scanInfo.contract} · Base chain 8453 · {scanInfo.count} verified wallet-owned NFT{scanInfo.count===1?'':'s'} found.</span></div>}

        {assets.length>0&&<>
          <div className={styles.sectionHead}><small>STEP 2 · CHOOSE EXACTLY 3</small><h2>Pick the three parents.</h2><p>Tap a card to select it. These production NFTs are only references; the test will mint separate Common copies on Sepolia.</p></div>
          <div className={styles.assetGrid}>{assets.map(asset=>{
            const chosen=selectedIds.includes(asset.tokenId);
            return <button key={asset.tokenId} type="button" className={`${styles.asset} ${chosen?styles.selected:''}`} onClick={()=>toggle(asset.tokenId)} disabled={imported.length>0}>
              <span className={styles.check}>{chosen?'✓':'+'}</span>
              {asset.imageUrl?<img src={asset.imageUrl} alt={asset.name}/>:<div className={styles.placeholder}>VOXELFLIP #{asset.tokenId}</div>}
              <div className={styles.assetBody}><small>REAL BASE NFT · #{asset.tokenId}</small><b>{asset.name}</b><span>{asset.animationUrl?'3D/animation metadata detected':'NFT metadata verified'}</span></div>
            </button>;
          })}</div>
          <div className={styles.selectionBar}><div><b>{selected.length} / 3 selected</b><span>{selected.length===3?'Ready to verify your test Forge.':'Choose three distinct VoxelFlips.'}</span></div>{selected.length>0&&imported.length===0&&<button className={styles.secondary} onClick={()=>setSelectedIds([])}>CLEAR SELECTION</button>}</div>
        </>}

        {scanInfo&&assets.length<3&&<div className={styles.notice}><b>YOU NEED 3 DISTINCT VOXELFLIPS</b><span>This wallet currently has {assets.length}. Make and mint {Math.max(0,3-assets.length)} more real VoxelFlip{3-assets.length===1?'':'s'}, then come back and tap Scan again.</span></div>}

        {selected.length===3&&<>
          <div className={styles.sectionHead}><small>STEP 3 · VERIFY TEST FORGE</small><h2>Review the destination before anything is written.</h2><p>This is your already-deployed Base Sepolia creator Forge. MetaMask may switch networks here, but verifying it does not spend ETH.</p></div>
          <div className={styles.inputRow}><input value={cloneAddress} onChange={e=>{setCloneAddress(e.target.value);setCloneVerified(false)}} placeholder="Base Sepolia Forge clone 0x…" autoCapitalize="off" autoCorrect="off" spellCheck="false"/><button className={styles.secondary} onClick={verifyClone} disabled={busy}>{cloneVerified?'✓ VERIFIED':'VERIFY FORGE'}</button></div>
          <div className={styles.review}>{selected.map((asset,index)=><article key={asset.tokenId}><small>PARENT {index+1}</small><b>{asset.name}</b><span>Production VoxelFlip #{asset.tokenId}<br/>{short(asset.contract)}</span></article>)}</div>
          {cloneVerified&&<button className={styles.primary} onClick={importSelected} disabled={busy||imported.length===3}>{imported.length===3?'3 TEST COPIES IMPORTED':busy?'WAITING FOR METAMASK…':'IMPORT 3 METADATA COPIES TO SEPOLIA'}</button>}
          <p className={styles.fine}>Import means minting three separate Common NFTs inside the test Forge with the same tokenURI metadata. It does not move the production NFTs.</p>
        </>}
      ...
      {imported.length===3&&<section className={styles.panel}>
        <div className={styles.confirmed}><b>✓ 3 REAL-METADATA COMMON COPIES READY</b><span>{imported.map(item=>`VoxelFlip #${item.tokenId} → Test Common #${item.testTokenId}`).join(' · ')}</span>{importTx&&<a href={explorerTx(importTx)} target="_blank" rel="noreferrer">VIEW IMPORT TRANSACTION ↗</a>}</div>
        <div className={styles.sectionHead}><small>STEP 4 · SIGN + FORGE</small><h2>Now run the real lineage 3→1.</h2><p>First MetaMask shows a ForgeRequest signature. Then it separately shows the Base Sepolia transaction with the live merge fee. The three TEST copies are burned; the three production NFTs are not.</p></div>
        <button className={styles.primary} onClick={forgeRare} disabled={busy||!!rare}>{rare?'RARE CONFIRMED':busy?'WAITING FOR SIGNATURE / METAMASK…':'SIGN + FORGE THESE 3 INTO A RARE'}</button>
      </section>}

      {rare&&<section className={styles.panel}>
        <div className={styles.confirmed}><b>✓ REAL-VOXEL LINEAGE TEST COMPLETE · RARE #{rare.tokenId}</b><span>Three verified production VoxelFlip metadata records entered the Sepolia test flow, their Common copies were burned, and one Rare descendant was minted. Your production assets stayed untouched.</span><a href={explorerTx(rare.txHash)} target="_blank" rel="noreferrer">VIEW FORGE TRANSACTION ↗</a></div>
        <div className={styles.sectionHead}><small>STEP 5 · WHAT THIS PROVES</small><h2>The bridge works. The next leap is visual fusion.</h2><p>This Rare currently records the real parent lineage and inherits the first parent’s visual/3D media as a test reference. It does not yet generate a brand-new combined 3D model from the three parents. That generation layer is the next product milestone before any mainnet Forge design.</p></div>
        <div className={styles.review}>{imported.map((asset,index)=><article key={asset.tokenId}><small>SOURCE {index+1}</small><b>{asset.name}</b><span>Production #{asset.tokenId} · still on Base</span></article>)}</div>
        <p className={styles.fine}>No claim of resale value or profit is made by the Forge tier. Rarity here is contract state and lineage, not a guarantee of market price.</p>
      </section>}
    </div>
