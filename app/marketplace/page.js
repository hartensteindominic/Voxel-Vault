...
export default function MarketplacePage(){
 ...
 return <main className="cj-page">
   ...
   <section id="catalog" className="cj-catalog"><div className="cj-sectionTitle"><div><small>OBJECT INDEX</small><h2>Browse without the noise.</h2></div><p>{products.length} shown · open any object in the verified viewer above.</p></div><div className="cj-toolbar"><input className="cj-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search product, category or SKU…"/><select className="cj-select" value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(value=><option key={value} value={value}>{value==='all'?'All categories':value}</option>)}</select><select className="cj-select" value={status} onChange={e=>setStatus(e.target.value)} aria-label="Filter by readiness"><option value="all">All readiness states</option><option value="ready">Checkout ready</option><option value="verified">3D verified</option><option value="pending">3D pending / review</option></select>{filtered&&<button className="cj-clear" onClick={clearFilters}>Clear filters</button>}</div><div className="cj-results"><span>{products.length} result{products.length===1?'':'s'}</span><b>{filtered?'FILTERED VIEW':'ALL PRODUCTS'}</b></div>
   {products.length?<div className="cj-grid">{products.map((item,index)=>{const state=modelState(item);const itemReady=isReady(item);const active=selected?.id===item.id;return <CJProductCard key={item.id} item={item} index={index} active={active} state={state} ready={itemReady} onSelect={()=>choose(item)} onOpen={()=>choose(item,{top:true})}/>})}</div>:<div className="cj-empty">No products match those filters. <button onClick={clearFilters}>Show all products</button></div>}</section>
   ...
   <nav className="cj-mobileDock" aria-label="Marketplace navigation"><a href="#catalog"><b>⌕</b><span>Find</span></a><Link href="/scan"><b>⌗</b><span>Scan</span></Link><Link href="/vault"><b>◇</b><span>Vault</span></Link><Link href="/ai"><b>✦</b><span>AI</span></Link></nav>
