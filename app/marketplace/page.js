...
function selectedState(item){const state=modelState(item);if(state==='verified')return'EXACT MODEL APPROVED';if(state==='review')return'INTERACTIVE PREVIEW READY';return'PREVIEW BUILDING AUTOMATICALLY'}
...
export default function MarketplacePage(){
 ...
 return <main className="cj-page">
   ...
   <section className="cj-hero"><div><div className="cj-kicker">REAL CJ PRODUCTS · INTERACTIVE PREVIEWS BUILD AUTOMATICALLY</div><h1>Real products.<br/><em>Brought to life.</em></h1><p>Explore real CJ-sourced products as interactive objects. Voxel Vault starts likely previews early, keeps finished models available for return visits, and lets the build continue in the background while you are away.</p><div className="cj-trust"><span>{REAL_WORLD_CATALOG.length} CJ products synced</span><span>interactive previews build automatically</span><span>{ready} live checkout</span><span>USD · no wallet to buy</span></div><div className="cj-explainer"><b>No customer verification step.</b><span>Voxel Vault handles product matching and readiness internally. You browse, inspect the object, and buy only when the physical product and matching digital collectible are ready.</span></div></div>{selected&&<div className="cj-feature"><div className="cj-featureTop"><small>SELECTED · {selected.supplierSku}</small><b>{selectedState(selected)}</b></div><RealWorld3DNFT item={selected} hero/><div className="cj-featureFoot"><div><small>{selected.type}</small><strong>{selected.name}</strong><span>{isReady(selected)?'READY TO BUY':'INTERACTIVE PREVIEW · CHECKOUT OPENS WHEN READY'}</span></div><b>${selected.customerPriceUsd}</b></div><div className="cj-featureActions">{isReady(selected)?<Link className="cj-primary" href={`/marketplace?purchase=${encodeURIComponent(selected.purchaseAssetId)}`}>Buy with card</Link>:<span className="cj-locked">Checkout opens automatically when ready</span>}<a className="cj-secondary" href={selected.sourceUrl} target="_blank" rel="noreferrer">View CJ listing ↗</a></div></div>}</section>
   <section id="catalog" className="cj-catalog"><div className="cj-sectionTitle"><div><small>PRODUCT INDEX</small><h2>Choose an object.</h2></div><p>{products.length} shown · likely previews start before selection and can finish in the background.</p></div><div className="cj-toolbar"><input className="cj-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search product, category or SKU…"/><select className="cj-select" value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(value=><option key={value} value={value}>{value==='all'?'All categories':value}</option>)}</select><select className="cj-select" value={status} onChange={e=>setStatus(e.target.value)} aria-label="Filter by readiness"><option value="all">All products</option><option value="ready">Ready to buy</option><option value="verified">Exact model approved</option><option value="pending">Preview building / review</option></select></div>
   ...
   <section className="cj-policy"><small>HOW IT WORKS</small><h2>Previews build in the background. Buying stays simple.</h2><p>Voxel Vault syncs CJ media, starts likely model jobs early, stores completed results for reuse, checks product accuracy internally, confirms fulfillment, and then enables card checkout. If an AI-generated object does not closely match the physical product, it remains a preview and is never promoted as the official collectible.</p><div className="cj-pipeline"><span>01 · CJ SYNCED</span><i>→</i><span>02 · PREVIEW BUILT</span><i>→</i><span>03 · MODEL APPROVED</span><i>→</i><span>04 · FULFILLMENT READY</span><i>→</i><span>05 · BUY WITH CARD</span></div></section>
   <footer className="cj-footer"><span>VOXEL VAULT · REAL PRODUCTS / DIGITAL COLLECTIBLES</span><Link href="/">Home</Link><span>NO CRYPTO REQUIRED</span></footer>
 </main>
}
