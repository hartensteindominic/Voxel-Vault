...
export default function VaultPage() {
  ...
  const [note, setNote] = useState('Your properties, purchases, and VoxelPop creations live here.');
  ...
  useEffect(() => {
    ...
    window.addEventListener('focus', refresh); window.addEventListener('storage', refresh); window.addEventListener('voxel-vault:property-draft-saved', refresh);
    return () => { window.removeEventListener('focus', refresh); window.removeEventListener('storage', refresh); window.removeEventListener('voxel-vault:property-draft-saved', refresh); };
  }, []);
  ...
  useEffect(() => {
    ...
    async function apply(client, nextSession) {
      ...
      if (!nextSession?.user) {
        setPurchasedTwins([]); refreshLocal(); setNote('Saved items on this device are shown below. Sign in to restore purchased twins and sync across devices.'); return;
      }
      setSyncing(true);
      try {
        const [cloudProperties, cloudVoxels, ownedTwins] = await Promise.all([
          ...
          fetch('/api/digital-estates/mine', { cache: 'no-store', headers: { Authorization: `Bearer ${nextSession.access_token}` } }).then(async (response) => {
            const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.error || 'Purchased Digital Twins could not be loaded.'); return Array.isArray(data.owned) ? data.owned : [];
          }).catch(() => []),
        ]);
        if (!active) return;
        const mergedProperties = mergePropertyDraftRecords(cloudProperties, readPropertyDrafts());
        replaceLocalPropertyDrafts(mergedProperties); setProperties(mergedProperties); setVoxelRecords(mergeVoxelRecords(cloudVoxels, readLocalVoxelRecords())); setPurchasedTwins(ownedTwins);
        setNote('Synced. Your Test Buy references, saved properties, purchased twins, and 3D creations are together here.');
      } catch (error) { if (active) setNote(error instanceof Error ? error.message : 'Account sync is unavailable. Your local Vault still works.'); }
      finally { if (active) setSyncing(false); }
    }
    getSupabaseBrowserAsync().then(async (client) => {
      if (!active) return; clientRef.current = client; const { data } = await client.auth.getSession(); await apply(client, data.session || null);
  ...
  async function signIn() {
    try { const client = clientRef.current || await getSupabaseBrowserAsync(); clientRef.current = client; const { error } = await client.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: new URL('/vault', window.location.origin).toString() } }); if (error) throw error; }
    catch (error) { setNote(error instanceof Error ? error.message : 'Could not start sign-in.'); }
  }

  return <main className="page"><div className="shell">
    <nav className="top"><Link href="/" className="brand"><span>V</span><b>VOXEL VAULT</b></Link><div><Link href="/property">Create</Link><Link href="/world">World</Link><Link href="/more">More</Link></div></nav>
    <header className="hero"><small>MY VAULT</small><h1>Your properties.<br/><em>Your voxels.</em></h1><p>{note}</p><div className="heroActions"><Link href="/property?source=properties" className="primary">Create from My Properties →</Link>{session?.user ? <Link href="#purchased-twins">My purchased twins →</Link> : null}{!session?.user ? <button type="button" onClick={signIn}>Sync with Google</button> : <span className="synced">✓ {syncing ? 'SYNCING' : 'SIGNED IN'}</span>}</div></header>

    <section className="section" id="purchased-twins">
      <div className="sectionHead"><div><small>MY PURCHASED TWINS</small><h2>Bought it? Make the voxel.</h2></div><Link href="/vault/estates/mine">Purchase details →</Link></div>
      {purchasedTwins.length ? <div className="propertyGrid">{purchasedTwins.map((item) => {
        const estate = item.estate || {}; const voxelReady = Boolean(item.voxelReady && item.voxelTaskId && item.voxelModelUrl);
        ...
        return <article className="propertyCard" key={estate.id}><div className="propertyVisual"><div className="miniScene"><i/><i/><b/></div><span>{voxelReady ? 'PURCHASED · 3D VOXEL READY' : 'PURCHASED DIGITAL TWIN'}</span></div><div className="propertyBody"><small>ACCOUNT-SECURED DIGITAL PURCHASE</small><h3>{estate.name || 'Purchased Digital Twin'}</h3><p>{voxelReady ? 'Your exact saved 3D voxel is attached to this purchase. Open or remake it, or mint the finished voxel.' : 'You already bought this Digital Twin. Its custom VoxelPop 3D voxel is included—no second property-creation charge.'}</p><div className="cardActions"><Link href={`/vault/estates/${encodeURIComponent(estate.id)}/voxel`} className="make3d">{voxelReady ? 'Open / remake 3D Voxel' : 'Create my 3D Voxel · included'}</Link><Link href="/vault/estates/mine" className="open3d">Purchase details</Link>{mintHref ? <Link href={mintHref} className="mint">Mint finished voxel</Link> : null}</div></div></article>;
      })}</div> : <div className="smallEmpty">{session?.user ? <>No account-secured Digital Twin purchases are attached to this account yet. Your Test Buy and saved properties are still shown below.</> : <>Sign in to restore account-secured purchases. Your local Test Buy and saved properties remain below.</>}</div>}
    </section>

    <section className="section"><div className="sectionHead"><div><small>MY PROPERTIES</small><h2>Pick one. Make it 3D.</h2></div><Link href="/geo/slice">Try $1.99 demo →</Link></div>
      {shownProperties.length ? <div className="propertyGrid">{shownProperties.map((property) => {
        const modelReady = Boolean(property?.visual?.modelUrl || property?.voxelpop?.modelUrl); const taskId = String(property?.visual?.modelTaskId || property?.voxelpop?.modelTaskId || ''); const creationDraftId = String(property?.voxelpop?.creationDraftId || '');
        const mintReady = modelReady && taskId.startsWith('local-v1:') && creationDraftId; const collected = property?.commerce?.kind === 'property_voxel_collectible' && property?.commerce?.status === 'paid';
        return <article className={`propertyCard ${property.demoOnly ? 'demo' : ''}`} key={property.id}><div className="propertyVisual">{property?.visual?.thumbnailUrl ? <img src={property.visual.thumbnailUrl} alt=""/> : <div className="miniScene"><i/><i/><b/></div>}<span>{propertyStatus(property)}</span></div><div className="propertyBody"><small>{property.demoOnly ? 'TEST BUY · NOT REAL OWNERSHIP' : collected ? 'PURCHASED DIGITAL PROPERTY' : 'PROPERTY SOURCE'}</small><h3>{property.label || 'Saved property'}</h3><p>{propertyCopy(property)}</p>{property.demoOnly && property?.demoPurchase?.priceCents ? <div className="priceLine"><b>{money(property.demoPurchase.priceCents)}</b><span>demo credit</span></div> : null}{collected && property?.commerce?.priceCents ? <div className="priceLine"><b>{money(property.commerce.priceCents)}</b><span>{property.commerce.priceLabel || 'digital item'}</span></div> : null}<div className="cardActions">{modelReady && !property.demoOnly ? <Link href={`/vault/property-drafts/${encodeURIComponent(property.id)}`} className="open3d">Open 3D</Link> : null}<Link href={propertyCreateHref(property)} className="make3d">{modelReady ? 'Re-create 3D Voxel' : 'Create 3D Voxel'}</Link>{mintReady ? <Link href={`/property/mint?draftId=${encodeURIComponent(creationDraftId)}&taskId=${encodeURIComponent(taskId)}&name=${encodeURIComponent(property.label || 'VoxelPop Property')}`} className="mint">Mint digital voxel</Link> : null}</div></div></article>;
      })}</div> : <div className="empty"><div className="emptyIcon">⌂</div><h3>No properties in your Vault yet.</h3><p>Start with a photo or try the demo property selector. Once a property is here, you can use it as the source for the 3D preview → voxel → optional mint flow.</p><Link href="/property">Add my first property →</Link></div>}
    </section>

    <section className="section creationsSection"><div className="sectionHead"><div><small>MY 3D CREATIONS</small><h2>Finished digital things.</h2></div><Link href="/studio">Open Studio →</Link></div>{creations.length ? <div className="creationGrid">{creations.slice(0, 12).map((creation) => <article className="creationCard" key={creation.sessionId}><div className="creationImage"><img src={creation.image} alt=""/><span>{creation.mint?.tokenId ? 'MINTED' : creation.modelUrl ? '3D READY' : 'IMAGE'}</span></div><div><h3>{creation.name}</h3><p>{creation.mint?.tokenId ? `VoxelFlip #${creation.mint.tokenId}` : creation.modelUrl ? 'Movable 3D model ready' : 'VoxelPop image saved'}</p></div></article>)}</div> : <div className="smallEmpty">Your non-property VoxelPop creations will appear here. <Link href="/studio">Create one →</Link></div>}</section>
    <section className="extras"><div><small>OPTIONAL</small><h2>Need the other tools?</h2><p>Wallets, marketplace tools, property verification, investment-provider screens, and owner controls are kept out of the main Vault so your collection stays understandable.</p></div><Link href="/more">Open More →</Link></section>
    <footer>A VoxelPop creation, demo property, purchased Digital Twin, payment, map marker, or NFT is a digital record—not a deed, title, rent right, security, or ownership of the physical property.</footer>
  </div><style jsx>{`
