...
export default function PurchasedTwinVoxelPage() {
  ...
  const stage = finalReady ? 4 : previewApproved ? 3 : sourceAccepted ? 2 : 1; const labels = ['SOURCE', '3D VOXEL PHOTO', 'VOXEL', 'MINT'];
  ...
  function acceptPhoto() { if (sourceKind !== 'photo' || !rightsConfirmed) return setStatus('Confirm that you took this photo or have permission to use it.'); setSourceAccepted(true); setPreviewReady(false); setStatus('Photo approved. Loading the photo-matched 3D voxel photo first.'); }
  function approvePreview() { if (!previewReady) return; setPreviewApproved(true); setFinal3d({ status: 'IN_PROGRESS', taskId: null, modelUrl: null }); setStatus('3D voxel photo approved. Building the separate movable voxel now…'); }
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    {stage === 1 ? <div className={styles.choice}><button className={styles.primary} type="button" onClick={usePurchasedDesign}>Use the twin I bought → 3D voxel photo</button><button className={styles.upload} type="button" onClick={choosePhoto} disabled={busy === 'photo'}>{busy === 'photo' ? 'Preparing photo…' : 'Use my property photo instead'}</button><input ref={inputRef} className={styles.hidden} type="file" accept="image/*,.heic,.heif" onChange={selectPhoto}/>{sourceKind === 'photo' ? <><label className={styles.rights}><input type="checkbox" checked={rightsConfirmed} onChange={(event) => setRightsConfirmed(event.target.checked)}/><span>I took this photo or have permission to use it.</span></label><button className={styles.secondary} type="button" onClick={acceptPhoto} disabled={!rightsConfirmed}>Use this photo → 3D voxel photo</button></> : null}</div> : null}
    {stage === 2 ? <><p className={styles.copy}>See the 3D voxel photo first and compare it with the source. The movable voxel does not start until you approve this review.</p><div className={styles.viewer}><PhotoReliefModelViewer imageUrl={sourceUrl} onReady={() => setPreviewReady(true)}/><span className={styles.badge}>{sourceKind === 'photo' ? 'YOUR PHOTO · 3D VOXEL PHOTO' : 'PURCHASED TWIN · 3D VOXEL PHOTO'}</span></div><div className={styles.choice}><button className={styles.primary} type="button" onClick={approvePreview} disabled={!previewReady}>{previewReady ? 'Looks right → Build my movable 3D voxel' : 'Building 3D voxel photo…'}</button><button className={styles.upload} type="button" onClick={resetSource}>Change source</button></div></> : null}
