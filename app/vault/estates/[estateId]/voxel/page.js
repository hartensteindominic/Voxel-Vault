...
export default function PurchasedTwinVoxelPage() {
  ...
  const stage = finalReady ? 4 : previewApproved ? 3 : sourceAccepted ? 2 : 1; const labels = ['SOURCE', 'VOXEL PHOTO', 'MOVABLE VOXEL', 'MINT'];
  ...
  function usePurchasedDesign() { if (!estate) return; setSourceUrl(estateDesignUrl(estate)); setSourceKind('purchased-design'); setRightsConfirmed(true); setSourceAccepted(true); setPreviewReady(false); setPreviewApproved(false); setFinal3d({ status: 'NOT_STARTED', taskId: null, modelUrl: null }); setStatus('Using the Digital Twin design you bought. Loading its 3D voxel photo first.'); }
  ...
  function acceptPhoto() { if (sourceKind !== 'photo' || !rightsConfirmed) return setStatus('Confirm that you took this photo or have permission to use it.'); setSourceAccepted(true); setPreviewReady(false); setStatus('Photo approved. Loading the recognizable 3D voxel photo first.'); }
  function approvePreview() { if (!previewReady) return; setPreviewApproved(true); setFinal3d({ status: 'IN_PROGRESS', taskId: null, modelUrl: null }); setStatus('3D voxel photo approved. Building the separate movable 3D voxel now…'); }
  ...
  const registerVoxel = useCallback(async (recipe) => {
    ...
    try {
      ...
      setFinal3d({ status: 'SUCCEEDED', taskId: data.taskId, modelUrl: data.modelUrl }); setStatus('Movable 3D voxel ready and attached to your purchased Digital Twin.');
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <section className={styles.hero}><small>YOU ALREADY BOUGHT THIS DIGITAL TWIN</small><h1>Turn it into<br/>your movable 3D voxel.</h1><p>No second $4.99 VoxelPop creation charge. Choose the purchased design itself, or add a photo you are allowed to use for a more personal visual match.</p></section>
    ...
    {stage === 1 ? <div className={styles.choice}><button className={styles.primary} type="button" onClick={usePurchasedDesign}>Use the twin I bought → Voxel photo</button><button className={styles.upload} type="button" onClick={choosePhoto} disabled={busy === 'photo'}>{busy === 'photo' ? 'Preparing photo…' : 'Use my property photo instead'}</button><input ref={inputRef} className={styles.hidden} type="file" accept="image/*,.heic,.heif" onChange={selectPhoto}/>{sourceKind === 'photo' ? <><label className={styles.rights}><input type="checkbox" checked={rightsConfirmed} onChange={(event) => setRightsConfirmed(event.target.checked)}/><span>I took this photo or have permission to use it.</span></label><button className={styles.secondary} type="button" onClick={acceptPhoto} disabled={!rightsConfirmed}>Use this photo → Voxel photo</button></> : null}</div> : null}
    {stage === 2 ? <><p className={styles.copy}>See the 3D voxel photo first. The movable voxel does not start until you approve this review.</p><div className={styles.viewer}><PhotoReliefModelViewer imageUrl={sourceUrl} onReady={() => setPreviewReady(true)}/><span className={styles.badge}>{sourceKind === 'photo' ? 'YOUR PHOTO · 3D VOXEL PHOTO' : 'PURCHASED TWIN · 3D VOXEL PHOTO'}</span></div><div className={styles.choice}><button className={styles.primary} type="button" onClick={approvePreview} disabled={!previewReady}>{previewReady ? 'Looks right → Build movable 3D voxel' : 'Building voxel photo…'}</button><button className={styles.upload} type="button" onClick={resetSource}>Change source</button></div></> : null}
    {stage === 3 ? <><p className={styles.copy}>Now VoxelPop converts the approved voxel photo into the separate movable block model. No Meshy credits are used.</p><div className={styles.viewer}><LocalVoxelModelViewer imageUrl={sourceUrl} sourceImageUrl={sourceUrl} onReady={registerVoxel}/><span className={styles.badge}>BUILDING MOVABLE 3D VOXEL</span></div>{final3d.status === 'LOCAL_ONLY' ? <div className={styles.done}><b>Voxel visible, but not saved yet.</b><span>Reload this page and rebuild before minting so the account binding is complete.</span></div> : null}</> : null}
    {stage === 4 ? <><p className={styles.copy}>Your purchased Digital Twin now opens the exact saved movable 3D voxel attached to this account.</p><div className={styles.viewer}><SavedVoxelModelViewer modelUrl={final3d.modelUrl}/><span className={styles.badge}>PURCHASED TWIN · EXACT SAVED MOVABLE 3D VOXEL</span></div><div className={styles.done}><b>Saved to your account ✓</b><span>You can reopen this exact voxel from Vault. Minting is optional and applies to the digital voxel only.</span><Link className={styles.primary} href={mintHref}>Mint this digital voxel →</Link><button className={styles.upload} type="button" onClick={resetSource}>Remake with another source</button><Link className={styles.secondary} href="/vault">Back to my Vault</Link></div></> : null}
