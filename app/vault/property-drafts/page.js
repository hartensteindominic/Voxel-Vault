...
export default function PropertyDraftsPage() {
  ...
  const [note, setNote] = useState('Your saved and collected digital property voxels.');
  ...
  return <main className="page">
    ...
    {drafts.length ? <section className="grid">{drafts.map((draft) => {
      ...
      return <article key={draft.id}>
        ...
        <div className="body"><small>{collected ? 'COLLECTED DIGITAL VOXEL' : draft.blockchain?.minted ? 'MINTED DIGITAL MODEL' : 'SAVED 3D MODEL'}</small><h2>{draft.label || 'Saved property reference'}</h2>{collected ? <div className="paid"><b>{draft.commerce?.priceLabel || 'VoxelPop Digital Voxel'}</b><strong>{dollars(draft.commerce?.priceCents)}</strong></div> : null}
          <div className="actions"><Link href={`/vault/property-drafts/${encodeURIComponent(draft.id)}`}>OPEN 3D</Link><button className={draft.world?.public ? 'active' : ''} onClick={() => toggleWorld(draft)} disabled={busy === draft.id}>{draft.world?.public ? 'PUBLIC WORLD' : 'SHARE TO WORLD'}</button><Link href="/vault/properties/claim">{collected ? 'VERIFY + MINT · OPTIONAL' : 'VERIFY + MINT'}</Link></div>
    ...
    })}</section> : <section className="empty"><div className="cube"><i/><i/><i/></div><b>No property voxels yet.</b><span>Choose one photo and VoxelPop will guide it through 3D, My World preview, and collection.</span><Link href="/property">+ CREATE FIRST VOXEL</Link></section>}

    <p className="truth">Vault is your digital inventory. A collected voxel, payment, World marker, or NFT does not itself transfer deed/title, rent, fractional investment, occupancy, or other rights in the physical property. Verify &amp; Mint stays separate and optional.</p>
    <style jsx>{`
