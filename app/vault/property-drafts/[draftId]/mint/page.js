  ...
  useEffect(() => {
    ...
    async function apply(next) {
      ...
      try {
        ...
        setMessage(found
          ? 'Inspect the saved movable voxel one more time, then mint only if you want the digital NFT.'
      ...
      {draft ? <>
        <section className="assetCard">
          <div className="modelFrame">
            {modelUrl ? <MeshyModelViewer modelUrl={modelUrl}/> : <div className="modelMissing">3D model not available</div>}
          </div>
