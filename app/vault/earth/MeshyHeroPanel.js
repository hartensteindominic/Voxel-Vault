...
export default function MeshyHeroPanel({ building, listing = null }) {
  ...
  function useLicensedFeedReferences() {
    if (!feedReferences.length) return;
    const next = feedReferences.map((item) => ({
      url: item.url,
      rightsBasis: item.rightsBasis,
      rightsReference: item.rightsReference,
      uploadedName: item.label || 'Licensed provider view',
      uploading: false,
    }));
  ...
  return <section className="meshPanel">
    ...
    {!mesh?.displayModelUrl && ownerState === 'ready' ? <div className="referenceBuilder">
      <div className="builderIntro"><b>2–4 RIGHTS-CLEARED VIEWS</b><span>Front view first. iPhone HEIC/WebP/JPEG/PNG selections are normalized in-browser to a high-quality JPEG before private upload. Nothing generates until you press the final button.</span></div>
      ...
      <p className="meshFine">Google Maps/Earth/Street View, Zillow, Redfin and Apartments.com are live comparison/reference surfaces, not automatic Meshy inputs. Use user-owned, open-licensed, or explicitly derivative-licensed photos for reconstruction.</p>
    </div> : null}
