...
function modelPresentation(modelUrl, explicitPreview = '') {
  const raw = String(modelUrl || '').trim();
  const fallback = { assetUrl: raw, previewImageUrl: String(explicitPreview || '').trim(), repairUrl: '' };
  if (!raw) return fallback;
  try {
    const url = new URL(raw, typeof window !== 'undefined' ? window.location.origin : 'https://voxelvault.local');
    const metadata = new URLSearchParams(url.hash.replace(/^#/, ''));
    const previewImageUrl = String(explicitPreview || metadata.get('vvPreview') || '').trim();
    const repairUrl = String(metadata.get('vvRepair') || '').trim();
    url.hash = '';
    const assetUrl = raw.startsWith('/') ? `${url.pathname}${url.search}` : url.toString();
    return { assetUrl, previewImageUrl, repairUrl };
  } catch {
    return fallback;
  }
}
...
export default function MeshyModelViewer({ modelUrl, previewImageUrl = '', onModelError = null }) {
  ...
  const presentation = modelPresentation(modelUrl, previewImageUrl);
  ...
  useEffect(() => {
    ...
    let activeModelUrl = presentation.assetUrl;
    ...
    Promise.all([
      ...
      async function repairCachedModel() {
        if (recoveryRequested || !presentation.repairUrl) return false;
        recoveryRequested = true;
        setError('Repairing cached 3D from the completed Meshy job…');
        try {
          const client = await getSupabaseBrowserAsync();
          const { data } = await client.auth.getSession();
          const token = data.session?.access_token || '';
          if (!token) return false;
          const response = await fetch(presentation.repairUrl, {
            cache: 'no-store',
            headers: { Authorization: `Bearer ${token}` },
          });
          const repaired = await response.json().catch(() => ({}));
          const repairedUrl = repaired?.modelUrl || repaired?.displayModelUrl || '';
          if (!response.ok || !repairedUrl) return false;
          const next = modelPresentation(repairedUrl, presentation.previewImageUrl);
          if (!next.assetUrl) return false;
          activeModelUrl = next.assetUrl;
          loadAttempt = 0;
          setError('3D cache repaired. Loading interactive model…');
          retryTimer = window.setTimeout(loadModel, 220);
          return true;
        } catch {
          return false;
        }
      ...
      const loadModel = () => {
        ...
        loader.load(activeModelUrl, (gltf) => {
          ...
          root.add(model);
          setError('');
          const previewDelay = presentation.previewImageUrl ? Math.max(0, 700 - (Date.now() - startedAt)) : 0;
          ...
          const repairing = await repairCachedModel();
          if (repairing || dead) return;
          readyRef.current = false;
          setReady(false);
          setError(presentation.previewImageUrl
            ? 'Interactive 3D is temporarily unavailable. The rendered 3D image stays visible.'
  ...
  }, [presentation.assetUrl, presentation.previewImageUrl, presentation.repairUrl]);
