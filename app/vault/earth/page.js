...
import OpenRealityPanel from './OpenRealityPanel';
...
function sourceMode(atlas) {
  if (atlas?.sourceStatus?.authoritativeLocal) return { label: 'ERIE LOCAL AUTHORITY' };
  if (!atlas?.sourceStatus) return { label: 'WAITING FOR REGION' };
  if (atlas.sourceStatus.primary === 'overture-pmtiles' && !atlas.sourceStatus.fallbackUsed) return { label: 'OVERTURE PRIMARY' };
  return { label: 'OSM FALLBACK' };
}
...
export default function EarthPropertiesPage() {
  ...
  async function loadOpenImagery(lat, lng) {
    ...
    setOpenBusy(true);
    try {
      const response = await fetch(`/api/world-atlas/open-imagery?lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}&radius=140`);
      const data = await response.json().catch(() => ({}));
      setOpenImagery(data);
    } catch (error) {
      setOpenImagery({ ok: false, photos: [], meshyReferences: [], provider: 'KartaView', license: 'CC BY-SA 4.0', error: String(error?.message || error) });
    } finally {
      setOpenBusy(false);
    }
  }
  ...
  async function loadListings(params = {}, nextCategory = category, nextType = type) {
    ...
    try {
      ...
      setListings([]);
      setSelectedId('');
      setListingMessage(String(error?.message || error || 'Market search failed. Open-world exploration remains available.'));
    } finally {
  ...
  async function loadAtlas(params = {}) {
    ...
    try {
      ...
      if (Number.isFinite(Number(data.latitude)) && Number.isFinite(Number(data.longitude))) {
        ...
        setFocus((current) => ({ ...current, lat, lng, resolved: true }));
        loadOpenImagery(lat, lng);
      }
      const source = data?.sourceStatus?.fallbackUsed ? 'OpenStreetMap fallback' : 'Overture';
      setAtlasMessage(data.buildingCount
        ? `${data.buildingCount} source-backed building${data.buildingCount === 1 ? '' : 's'} loaded from ${source}. Free KartaView imagery is checked separately.`
        : 'The location resolved, but the global map sources returned no building footprint here. Nothing was invented.');
      setResetKey((value) => value + 1);
      return data;
    } catch (error) {
      setAtlas(null);
      setSelectedAtlasId('');
      setAtlasMessage(`${String(error?.message || error || 'World atlas lookup failed.')} Open imagery and authorized market search remain separate.`);
      return null;
    } finally {
      setAtlasBusy(false);
    }
  ...
  async function explore(params = {}) {
    ...
    if (Number.isFinite(params?.lat) && Number.isFinite(params?.lng)) {
      ...
      loadOpenImagery(Number(params.lat), Number(params.lng));
    } else {
  ...
  async function exploreAuthoritative(location) {
    ...
    try {
      ...
      setAtlasMessage(data.localBuildingStatus === 'parcel_linked_building'
        ? `1047 is anchored to the City parcel and a parcel-linked Erie County BUILDING footprint. ${data.atlas?.buildingCount || 1} nearby building${Number(data.atlas?.buildingCount || 1) === 1 ? '' : 's'} available.`
        : '1047 is anchored to the authoritative parcel. Erie does not provide an exact parcel-linked BUILDING footprint here, so no house geometry is promoted to verified.');
      await Promise.allSettled([loadListings({ lat, lng, query: location.query }), loadOpenImagery(lat, lng)]);
    } catch (error) {
      setAuthoritativeEvidence(null);
      setBuffaloReference(null);
  ...
  function nearMe() {
    if (!navigator.geolocation) return setAtlasMessage('Location services are unavailable in this browser.');
    setAtlasBusy(true);
  ...
  function chooseAtlas(atlasId) {
    const building = atlasBuildings.find((item) => item.atlasId === atlasId);
    setSelectedAtlasId(atlasId);
    setResetKey((value) => value + 1);
    if (building) loadOpenImagery(building.latitude, building.longitude);
  }
  ...
  function downloadRegion() {
    ...
    const geojson = {
      ...
      features: atlasBuildings.map((building) => ({ type: 'Feature', id: building.atlasId, geometry: building.geometry, properties: {
        ...
        sourceAuthority: building.source?.authority || null, sourceLicense: building.source?.license || null,
  ...
  function voxelStage(compactLabel = false) {
    if (hasAuthoritativeBuilding) return <div className={styles.stageCard}><BuffaloCalibratedReferenceModel reference={atlasReference} authoritativeTwin={authoritativeEvidence?.twin || null} buffaloReference={buffaloReference} addressLabel={focus.label} viewMode="orbit" resetKey={resetKey}/>{compactLabel ? <span className={styles.paneLabel}>SOURCE + LOCAL AUTHORITY</span> : null}</div>;
    if (atlasReference?.found) return <div className={styles.stageCard}><GeoReferenceModel reference={atlasReference} authoritativeTwin={null} viewMode="orbit" resetKey={resetKey}/>{compactLabel ? <span className={styles.paneLabel}>SOURCE-BACKED VOXEL</span> : null}</div>;
    return <div className={`${styles.stageCard} ${styles.stageEmpty}`}><b>{atlasBusy ? 'RESOLVING PROPERTY EVIDENCE…' : 'NO VERIFIED BUILDING FOOTPRINT'}</b><span>The parcel/location and open street evidence can still be inspected. Voxel Vault will not invent a house just to fill this panel.</span></div>;
  }

  return <main className={styles.page}>
    <header className={styles.header}><Link className={styles.brand} href="/vault">VOXEL VAULT</Link><nav className={styles.nav}><Link href="/geo">GEO</Link><Link href="/vault/estates/mine">MY TWINS</Link><Link href="/vault/properties/claim">VERIFY</Link></nav></header>

    <section className={styles.hero}>
      <div className={styles.eyebrow}><i className={styles.pulse}/> VOXEL VAULT WORLD ATLAS · NO PAID MAP KEY REQUIRED</div>
      <h1>Map the real world.<br/><em>Rebuild the best parts in 3D.</em></h1>
      <p className={styles.heroText}>One address across jurisdiction parcel evidence, Overture/OSM geometry, free open street imagery, the world Globe, authorized listings and selective Meshy 7 reconstruction. Missing evidence stays missing instead of being replaced with a fake house.</p>
      <form className={styles.search} onSubmit={submit}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Address, city, postcode · or latitude, longitude" aria-label="Search Earth"/><button disabled={atlasBusy || listingBusy}>{atlasBusy ? 'RESOLVING…' : 'EXPLORE'}</button><button type="button" onClick={nearMe} disabled={atlasBusy}>NEAR ME</button></form>
      ...
      <div className={styles.capabilities}>
        ...
        <div className={styles.capability}><b>OPEN STREET</b><span>{capabilityLabel(capabilities, 'openStreetReality')}</span></div>
    ...
    <section className={styles.shell}>
      <div className={styles.topbar}>
        ...
        <div className={styles.tabs} role="group" aria-label="Property visualization mode"><button className={view === 'compare' ? styles.active : ''} onClick={() => setView('compare')}>COMPARE</button><button className={view === 'street' ? styles.active : ''} onClick={() => setView('street')}>STREET</button><button className={view === 'voxel' ? styles.active : ''} onClick={() => setView('voxel')}>VOXEL</button><button className={view === 'globe' ? styles.active : ''} onClick={() => setView('globe')}>GLOBE</button></div>
      </div>

      <div className={styles.atlasGrid}>
        <div className={styles.stage}>
          {view === 'compare' ? <div className={styles.compare}><div className={styles.comparePane}><OpenRealityPanel imagery={openImagery} loading={openBusy} label={visualLabel} latitude={visualReady ? visualLat : null} longitude={visualReady ? visualLng : null}/><span className={styles.paneLabel}>FREE OPEN STREET REALITY</span></div><div className={styles.comparePane}>{voxelStage(true)}</div></div> : null}
          {view === 'street' ? <OpenRealityPanel imagery={openImagery} loading={openBusy} label={visualLabel} latitude={visualReady ? visualLat : null} longitude={visualReady ? visualLng : null}/> : null}
          {view === 'voxel' ? voxelStage(false) : null}
          {view === 'globe' ? <div className={`${styles.stageCard} ${styles.globeWrap}`}><GlobalEarthGlobe listings={listings} selectedId={selected?.id || ''} onSelect={setSelectedId} atlasBuildings={atlasBuildings} selectedAtlasId={selectedAtlas?.atlasId || ''} onAtlasSelect={chooseAtlas} onLocation={globeLocation}/><div className={styles.globeHint}>DRAG · PINCH · TAP EARTH · PEACH = MAP · MINT = LISTING</div></div> : null}
        </div>

        <aside className={styles.side}>
          <div className={styles.badgeRow}><span className={styles.badge}>{mode.label}</span><span className={styles.badge}>{atlasBuildings.length} BUILDINGS</span><span className={styles.badge}>{openImagery?.count || 0} OPEN PHOTOS</span>{focus.authority ? <span className={styles.badge}>{focus.authority}</span> : null}</div>
          <small>SELECTED PLACE</small><h3>{selectedAtlas ? atlasBuildingLabel(selectedAtlas) : focus.label}</h3><p className={styles.coord}>{visualReady ? `${visualLat.toFixed(6)}, ${visualLng.toFixed(6)}` : 'RESOLVING EXACT LOCATION'}</p>
          <div className={styles.facts}><div className={styles.fact}><b>{buffaloReference?.stories || selectedAtlas?.tags?.levels || '—'}</b><span>FLOORS</span></div><div className={styles.fact}><b>{buffaloReference?.totalLivingAreaSqFt ? `${Math.round(buffaloReference.totalLivingAreaSqFt).toLocaleString()} ft²` : '—'}</b><span>CITY AREA REF</span></div><div className={styles.fact}><b>{selectedAtlas?.height?.referenceHeightMeters ? `${Number(selectedAtlas.height.referenceHeightMeters).toFixed(1)}m` : '—'}</b><span>DISPLAY HEIGHT</span></div><div className={styles.fact}><b>{authoritativeEvidence?.countyRecord?.buildingMatchStrategy || selectedAtlas?.source?.license || '—'}</b><span>GEOMETRY STATUS</span></div></div>
          <div className={styles.sourceNote}><b>FAIL-SAFE PROPERTY TRUTH</b><span>Street photos, map geometry, assessment characteristics, Meshy models, listing media, legal title and investment rights are separate evidence layers. MINTING RECOMMENDED AFTER VERIFICATION; minting never upgrades title.</span></div>
          ...
          {selectedAtlas?.source?.sourceUrl ? <a className={styles.sourceLink} href={selectedAtlas.source.sourceUrl} target="_blank" rel="noreferrer">OPEN MAP SOURCE ↗</a> : null}<Link className={styles.sourceLink} href="/vault/properties/claim">VERIFY OWNER · CREATE PROPERTY PASSPORT</Link>
        </aside>
      </div>

      <PropertyTruthStack building={selectedAtlas} authoritativeEvidence={authoritativeEvidence} buffaloReference={buffaloReference} listing={selected} openImagery={openImagery} meshyConfigured={Boolean(capabilities?.meshy?.configured)} focusAuthority={focus.authority || ''}/>
      <PropertyEvidencePanel listing={selected} building={selectedAtlas} fallbackLabel={focus.label}/>

      <div className={styles.meshZone}><div className={styles.meshCopy}><small>MESHY · THE PERFECT AMOUNT</small><h2>Use the API you already have where it matters.</h2><p>Normal Earth browsing spends zero Meshy credits. When 2–4 nearby KartaView frames exist, they can be loaded as CC BY-SA reconstruction references; otherwise use your own or explicitly derivative-licensed photos. The generated GLB is cached so reopening a property does not spend credits again.</p></div><MeshyHeroPanel building={selectedAtlas} listing={selected} openReferences={openImagery?.meshyReferences || []}/></div>
    </section>
    ...
    <section className={styles.market}>
      ...
      <div className={extra.filters}><div>{[['all','Buy + Rent'],['sale','For Sale'],['rent','For Rent']].map(([id,label]) => <button key={id} className={type === id ? extra.activeFilter : ''} onClick={() => chooseType(id)}>{label}</button>)}</div><div>{CATEGORIES.map(([id,label]) => <button key={id} className={category === id ? extra.activeFilter : ''} onClick={() => chooseCategory(id)}>{label}</button>)}</div></div>
      <div className={styles.marketGrid}>{listings.length === 0 ? <div className={styles.emptyMarket}><b>{configured === false ? 'MAP READY · MARKET FEED NOT CONNECTED' : listingBusy ? 'CHECKING AUTHORIZED MARKET…' : 'NO LIVE LISTINGS HERE'}</b><span>A mapped building is not automatically for sale. Try another place or connect an authorized provider.</span></div> : listings.map((item) => <article key={item.id} className={styles.listing}><button type="button" className={extra.cardSelect} onClick={() => chooseListing(item)}>{item.imageUrl ? <img src={item.imageUrl} alt="" referrerPolicy="no-referrer"/> : <div className={styles.listingPlaceholder}>{categoryLabel(item.category)}</div>}<div className={styles.listingBody}><small>{item.provider} · {item.country || 'Earth'}</small><strong>{money(item)}</strong><b>{item.address || 'Address from source'}</b><span>{[item.city,item.region,item.postalCode].filter(Boolean).join(', ')}</span><span>{item.beds != null ? `${item.beds} bd · ` : ''}{item.baths != null ? `${item.baths} ba · ` : ''}{item.livingAreaSqft ? `${Math.round(item.livingAreaSqft).toLocaleString()} sqft` : categoryLabel(item.category)}</span></div></button>{item.sourceUrl ? <a className={styles.marketLink} href={item.sourceUrl} target="_blank" rel="noreferrer">OPEN SOURCE LISTING ↗</a> : null}</article>)}</div>
    ...
    <section className={extra.governance}><article><small>ANTI-MONOPOLY STEWARDSHIP</small><h2>More digital claims, higher marginal fee.</h2><p>The proposed Voxel Vault stewardship schedule stays <b>linear, not exponential</b>: $1/year base + $0.25 per existing global claim + $0.75 per existing claim in the same local region. Regional cap: 20. No owner/admin exemption. Billing remains disabled until an authoritative claim ledger exists. This is not a government tax and does not create rights in physical property.</p></article><article><small>WHO OWNS THE WORLD MAP?</small><h2>Voxel Vault can own the atlas product—not the Earth.</h2><p>Voxel Vault can own its software, interface, original metadata, compliant Meshy cache and workflows. Overture, OpenStreetMap, KartaView, municipalities and listing providers keep their source data and licenses.</p></article></section>

    <footer className={styles.footer}><b>REALITY ≠ TITLE ≠ INVESTMENT</b><span>Physical-market value and digital twin resale value remain separate. Map geometry, open street photos, listing photos, Meshy models, legal ownership and investment rights stay separate evidence layers; none guarantees appreciation or income.</span></footer>
  </main>;
}
