...
export const alt = 'Voxel Vault — turn a house photo into a 3D voxel photo and movable voxel';
  ...
  return new ImageResponse(
    <div style={{ width: '100%', height: '100%', display: 'flex', background: '#fffaf0', color: '#17131d', fontFamily: 'Arial, sans-serif', padding: 54, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: '58%', display: 'flex', flexDirection: 'column', justifyContent: 'center', zIndex: 2 }}>
        ...
        <div style={{ fontSize: 66, lineHeight: .96, fontWeight: 900, letterSpacing: -4 }}>See your house as a voxel. Then make it movable.</div>
        <div style={{ display: 'flex', gap: 10, marginTop: 28 }}>
          {['PHOTO', 'VOXEL PHOTO', 'MOVABLE VOXEL'].map((label, index) => <div key={label} style={{ padding: '11px 14px', borderRadius: 999, background: index === 2 ? '#7138f5' : '#ffffff', color: index === 2 ? '#ffffff' : '#574e59', border: '1px solid #ded6e3', fontSize: 15, fontWeight: 800 }}>{label}</div>)}
        </div>
      ...
      <div style={{ width: '42%', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}>
        <div style={{ width: 360, height: 360, borderRadius: 42, background: '#21172c', border: '8px solid #ffffff', boxShadow: '0 24px 50px rgba(62,36,96,.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', top: 18, left: 18, padding: '10px 14px', borderRadius: 999, background: '#c9ff54', color: '#2f4509', fontSize: 16, fontWeight: 900 }}>COMPARE FIRST</div>
        </div>
      </div>
    </div>,
    size,
