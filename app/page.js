...
// Sign in -> authorized photo -> $4.99 digital creation checkout -> local voxel -> source-backed 3D map -> World -> optional digital collection/mint.
...
// Estimated asset value is not settled cash. Regulated money/property rails stay separate.
    ...
    <div className={styles.shell}>
      ...
      <header className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <p className={styles.lead}>Sign in, choose a photo you can use, then pay $4.99 for one digital VoxelPop creation. The voxel is built locally on your device with no Meshy credits, then you verify the address and explore its source-backed 3D map.</p>
          ...
          <p className={styles.heroFine}>Creation costs $4.99. Your source photo stays on-device. A wallet is optional. Collecting or minting an eligible mapped digital asset is a separate optional action.</p>
        </div>
      ...
      <section className={styles.unifiedCard}>
        ...
        <div className={styles.assetGrid}>
          <Link href="/property" className={`${styles.assetTile} ${styles.propertyTile}`}><div className={styles.tileIcon}>+</div><small>CREATE · $4.99</small><strong>Make a VoxelPop</strong><span>Paid local voxel creation plus source-backed mapped 3D.</span><b>Create →</b></Link>
          ...
          <Link href="/more" className={`${styles.assetTile} ${styles.nftTile}`}><div className={styles.tileIcon}>•••</div><small>MORE</small><strong>Optional tools</strong><span>Sandbox property, wallets, provider investments, rentals, AI and verification.</span><b>See More →</b></Link>
        </div>
      </section>

      <section className={styles.creationCard}>
        <div className={styles.step}><span>+</span><div><p>HOW IT WORKS</p><h2>Photo → $4.99 → voxel → mapped 3D.</h2><span>The authorized photo stays on your device. After the creation payment is verified, Voxel Vault makes the local stylized voxel; the interactive property map comes from source-backed geometry.</span></div></div>
        <Link className={styles.startButton} href="/property">START → SIGN IN + CREATE MY VOXEL</Link>
        <div className={styles.microFlow}><b>PHOTO</b><i>→</i><b>$4.99</b><i>→</i><b>VOXEL</b><i>→</i><b>3D</b><i>→</i><b>WORLD</b><i>→</i><b>OPTIONAL COLLECT + VAULT</b></div>
        <small>No Meshy credits are required for the normal property flow. A wallet is optional. Creation payment, collection, minting, trading and money movement remain explicit actions—nothing silently spends credits or moves funds.</small>
      </section>
