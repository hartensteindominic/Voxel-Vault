    ...
    <div className={styles.shell}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <h1>Turn your house photo<br/><em>into a movable voxel.</em></h1>
          ...
          <p className={styles.heroFine}>Your creation costs $4.99 once. After payment you inspect the 3D preview, approve it, and only then build the voxel. Your source photo stays on your device and the normal creation flow works without Meshy credits. No wallet is required to create.</p>
        </div>
      ...
      <section className={styles.flowCard} id="how-it-works" aria-label="VoxelPop creation steps">
        ...
        <div className={styles.microFlow}><b>PHOTO</b><i>→</i><b>$4.99 CREATE</b><i>→</i><b>3D PREVIEW</b><i>→</i><b>APPROVE</b><i>→</i><b>VOXEL</b><i>→</i><b>MINT</b></div>
        <Link className={styles.startButton} href="/demo">TRY THE PUBLIC SAMPLE →</Link>
        <small>Optional minting is a separate wallet action after the finished digital voxel is ready. A one-photo creation preserves what is visible in the source image but does not pretend to reconstruct unseen sides or exact survey-grade dimensions.</small>
      </section>

      <section className={styles.destinationSection}>
        <div className={styles.sectionIntro}><p>SEE THE PRODUCT</p><h2>Useful proof before checkout.</h2><span>No testimonials or fake customer examples are needed to understand the interaction. The built-in sample uses the same production 3D preview and local voxel viewers, while World and Vault show where a finished creation goes next.</span></div>
