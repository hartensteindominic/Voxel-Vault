...
function referenceFor(item) {
  ...
  return {
    ...
    source: { authority: item.mine ? 'Your saved Voxel Vault draft' : 'Voxel Vault member-shared 3D draft', license: '', sourceUrl: '' },
...
export default function WorldPage() {
    ...
    async function load(nextSession) {
      ...
      try {
        ...
        setMessage(nextSession?.user
          ...
          : shared.length ? `${shared.length} shared VoxelPop propert${shared.length === 1 ? 'y' : 'ies'}. Sign in to add your private Vault items.` : 'No public properties yet. Sign in and build the first one.');
      } catch (error) {
  ...
  return <main className="page">
    ...
    <section className="head"><small>✦ {signedIn ? 'MY WORLD + PUBLIC WORLD' : 'PUBLIC VOXEL WORLD'} ✦</small><h1>{signedIn ? <>Your properties.<br/><em>One little world.</em></> : <>Everyone’s voxels.<br/><em>One little world.</em></>}</h1><p>{message}</p></section>

    <section className="globe">
      <PlanetStreamGlobe listings={items} selectedId={selectedId} onSelect={setSelectedId} atlasBuildings={[]} simpleMode />
      {!items.length ? <div className="empty"><b>Your world is waiting.</b><Link href="/property">+ CREATE A VOXEL PROPERTY</Link></div> : null}
    ...
    {selected ? <section className="selected">
      ...
      <div className="info"><small>{selected.mine ? 'MY WORLD' : `SHARED BY ${selected.handle ? `@${selected.handle}` : selected.owner}`}</small><h2>{selected.label || '3D Property'}</h2><div className="badges"><span>{selected.purchasedDigitalCollectible ? 'COLLECTED' : selected.minted ? 'MINTED' : '3D'}</span><span>{selected.mine && selected.private ? 'PRIVATE' : 'WORLD'}</span><span>{selected.rightsVerified ? 'RIGHTS VERIFIED' : 'MODEL ONLY'}</span></div><p>{selected.mine ? 'This saved item is visible to you here even when it is not published publicly.' : 'This member chose to share the digital property model on public World.'}</p><Link href={selected.mine ? '/vault/property-drafts' : '/property'}>{selected.mine ? 'OPEN MY VAULT' : '+ CREATE YOUR PROPERTY'}</Link></div>
    </section> : null}

    <p className="truth">My World can show your private account-synced digital models at their saved locations. Public World only shows opt-in shared models with privacy-rounded coordinates. Neither a map marker, payment, voxel nor NFT proves deed/title, fractional ownership, rent, occupancy or other legal real-property rights.</p>
