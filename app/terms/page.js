...
export default function TermsPage() {
  return <main className={styles.page}><ProductTopNav/><div className={styles.shell}>
    <header className={styles.hero}><small>TERMS</small><h1>Digital creation.<br/>Clear boundaries.</h1><p>The current core Voxel Vault product turns an authorized property photo into a reviewable 3D voxel photo and then a separate movable voxel. Other property, wallet, map, and provider features have separate meanings and requirements.</p></header>
    <section className={styles.card}>
      ...
      <p>The 3D voxel photo and movable voxel are digital representations. A single photograph cannot truthfully establish unseen walls, exact roof geometry, precise dimensions, structural condition, boundaries, survey accuracy, appraisal value, or legal property rights. Results can vary with photo quality, device performance, and browser support.</p>
      <h2>Payments</h2>
      <p>A completed $4.99 VoxelPop creation checkout unlocks the 3D voxel photo review and movable-voxel creation flow for that paid creation. Separate optional digital collectible or blockchain actions, when offered, are not part of the physical-property purchase process.</p>
