...
function urlDraftId(value) {
  try {
    return new URL(value, window.location.origin).searchParams.get('draftId') || '';
  } catch {
    return '';
  }
}
  ...
  useLayoutEffect(() => {
    ...
    const bridgedFetch = async (input, init = {}) => {
      ...
      if (isGenerationCheckout && method === 'POST' && body instanceof FormData) {
        ...
        if (photo instanceof File && localDraftId) {
          await savePropertyCheckoutPhoto(localDraftId, photo);
        }
      }
