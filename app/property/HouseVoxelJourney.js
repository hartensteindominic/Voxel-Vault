...
const REFERENCE_MAX_DATA_LENGTH = 5_200_000;
const wait = (ms) => new Promise((resolve) => window.setTimeout(resolve, ms));
const emptyImage = () => ({ status: 'NOT_STARTED', progress: 0, imageUrl: null, taskId: null, taskToken: null, voxelImageTaskToken: null });
...
async function imageFromUrl(url) {
  ...
  await new Promise((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error('This house photo could not be opened. Try a screenshot instead.'));
  });
...
async function normalizeIphonePhoto(file) {
  ...
  try {
    ...
    const sourceW = Math.max(1, image.width || image.naturalWidth || 1);
    const sourceH = Math.max(1, image.height || image.naturalHeight || 1);
    const maxEdge = 2400;
...
async function prepareReferenceDataUrl(file) {
  const url = URL.createObjectURL(file);
  try {
    const image = await imageFromUrl(url);
    ...
    for (const quality of [0.9, 0.82, 0.74, 0.66, 0.58]) {
      ...
      if (data.length <= REFERENCE_MAX_DATA_LENGTH) return data;
    }
...
export default function HouseVoxelJourney() {
  ...
  async function pollVoxelImage(started, activeDraftId, iteration) {
    for (let attempt = 0; attempt < 120; attempt += 1) {
      ...
      const next = { ...started, ...data };
      setVoxelImage(next);
      if (data?.ready && data?.imageUrl) return next;
  ...
  async function saveFinishedVoxel({ activeDraftId, finalDone, voxelDone, lock }) {
    ...
    const finishedDraft = {
      ...
      voxelpop: {
        ...
        engine: 'voxelpop-provider-image-to-glb-v1',
  ...
  async function startDirectBuild({ photo, activeDraftId, lock }) {
    if (!photo || !activeDraftId || !session?.access_token) return;
    ...
    try {
      ...
      const reference = await prepareReferenceDataUrl(photo);
      const imageResponse = await fetch('/api/property-voxel-photo', {
        method: 'POST',
        headers: authHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ draftId: activeDraftId, reference }),
      });
      const imageStart = await imageResponse.json().catch(() => ({}));
      if (!imageResponse.ok || !imageStart?.ok || !imageStart?.taskId || !imageStart?.taskToken || !imageStart?.voxelImageTaskToken) {
        throw new Error(imageStart?.error || 'The voxel image could not start.');
      }
      const started = { status: 'PENDING', progress: 0, imageUrl: null, ...imageStart };
      setVoxelImage(started);
      const voxelDone = await pollVoxelImage(started, activeDraftId, iteration);
      ...
      const finalResponse = await fetch('/api/property-voxel-3d', {
        ...
        body: JSON.stringify({
          ...
          phase: 'voxel',
          voxelImageTaskId: voxelDone.taskId,
          voxelImageTaskToken: voxelDone.voxelImageTaskToken,
        }),
      });
  ...
  async function selectPhoto(event) {
    ...
    try {
      ...
      if (paidSessionId && draftId) {
        ...
        setBusy('');
        await startDirectBuild({ photo, activeDraftId: draftId, lock });
        return;
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPreviewFromFile(photo);
        setBusy('');
        await startDirectBuild({ photo, activeDraftId: data.draftId, lock });
      } catch (error) {
        if (active) {
  ...
  async function retryPaidBuild() {
    ...
    await startDirectBuild({ photo, activeDraftId: draftId, lock: propertyLock });
  }
  ...
  return <main className={styles.page}>
    <section className={styles.maker}>
      ...
      {generationStarted && !voxelReady && !finalReady ? <>
        ...
        <p className={styles.stepCopy}>{generationFailed ? 'Your paid property lock is safe. Retry the same build without paying again.' : 'VoxelPop is translating the house photo into the block-style image that drives the final 3D model.'}</p>
