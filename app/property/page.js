...
export default function SimplePropertyPage() {
  ...
  const draft = useMemo(() => buildPropertyDraft({ building, listing: null, fallbackLabel: query }), [building, query]);
  ...
  async function search(value = query) {
    ...
    try {
      ...
      setPlatform(status);
      if (!selected?.geometry) setMessage('Location found. No source-backed building footprint exists here yet, so Voxel Vault did not invent one.');
  ...
  return <main className="page">
    ...
    {building ? <section className="property">
      <div className="viewer">{reference ? <GeoReferenceModel reference={reference} authoritativeTwin={null} viewMode="orbit" resetKey={0}/> : <div className="empty3d">NO SOURCE-BACKED BUILDING YET</div>}</div>
      <div className="controls">
        <div className="title"><small>YOUR 3D PROPERTY</small><h1>{query}</h1><span>{building?.source?.authority || 'Source-backed world map'}</span></div>
