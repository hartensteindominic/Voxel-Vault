...
function previewStorageKey(draftId) { return `voxelpop-preview:${String(draftId || '').trim()}`; }
...
export default function PropertyPurchaseSuccessPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!response.ok || !data?.ok || data?.purchase?.paid !== true || !data?.building || !data?.model) {
        ...
        let localPreview = '';
        try { localPreview = window.localStorage.getItem(previewStorageKey(data.purchase.draftId)) || ''; } catch {}
        ...
        const mapBacked = data.model.mapBacked === true;
        const next = {
          ...base,
          label: data.purchase.address || base.label,
          fidelity: mapBacked ? 'map-backed-voxel-collectible' : 'photo-to-3d-to-voxel-collectible',
          state: 'paid-digital-collectible',
          visual: {
            ...(base.visual || {}),
            modelUrl: data.model.modelUrl || null,
            modelTaskId: data.model.taskId,
            thumbnailUrl: localPreview || data.model.thumbnailUrl || null,
            renderMode: mapBacked ? 'source-backed-map-3d' : 'generated-glb',
          },
          commerce: {
          ...
          legal: {
            ...
            note: mapBacked
              ? 'This paid VoxelPop item is a digital collectible rendered from source-backed map geometry. It is not a photo-derived survey or deed. Payment and any later mint do not transfer deed/title, rent, occupancy, investment or appreciation rights in the physical property.'
        ...
        try { window.localStorage.removeItem(previewStorageKey(data.purchase.draftId)); } catch {}
        if (!active) return;
        setSaved(true);
        setMessage(mapBacked ? 'Collected and saved to your Vault with source-backed 3D map geometry.' : 'Collected and saved to your Vault.');
      } catch (error) {
  ...
  return <main className="page">
    <section className="card">
      ...
      {result?.model?.modelUrl ? <div className="viewer"><MeshyModelViewer modelUrl={result.model.modelUrl}/><span>YOUR DIGITAL VOXEL</span></div> : result?.reference ? <div className="viewer"><GeoReferenceModel reference={result.reference} viewMode="orbit" resetKey={0}/><span>YOUR MAP-BACKED 3D VOXEL</span></div> : <div className="loading">Verifying payment → restoring map identity → saving your Vault item</div>}
      ...
      <p className="truth">You collected a digital VoxelPop item. A map-backed 3D item uses source-backed building geometry rather than a paid Meshy generation. The address, map, payment, or optional later mint does not create deed/title, rent, occupancy, fractional investment, appreciation, or other rights in the physical property.</p>
    </section>
