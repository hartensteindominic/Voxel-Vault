...
async function generateHouse({ imageUrl, requestId, requestRef }) {
...
export default function VoxelPopHouseImageGenerator({ imageUrl, onReady }) {
  ...
  const finish = (result) => {
    if (!result?.payload?.image) return;
    window[GLOBAL_RENDER_KEY] = result.payload.image;
    window.dispatchEvent(new CustomEvent('voxelpop-property-render-ready', { detail: { image: result.payload.image, draftId: result.draft.draftId } }));
    setRenderUrl(result.payload.image);
    setProvider(clean(result.payload.provider));
    setStatus('ready');
    callbackRef.current?.(result.payload.image);
  };
  ...
  useEffect(() => {
    ...
    generateHouse({ imageUrl, requestId, requestRef }).then((result) => {
      if (!dead && requestRef.current === requestId) finish(result);
  ...
  async function regenerate() {
    ...
    if (typeof window !== 'undefined') window[GLOBAL_RENDER_KEY] = '';
    try {
      const result = await generateHouse({ imageUrl, requestId, requestRef });
      if (requestRef.current === requestId) finish(result);
    } catch (generationError) {
