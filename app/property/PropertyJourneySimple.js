...
export default function PropertyJourneySimple() {
  ...
  async function startDirectBuild({ photo, activeDraftId, generationSessionId, lock }) {
    ...
    try {
      ...
      const imageResponse = await fetch('/api/property-photo-upload', {
        method: 'POST',
        headers: authHeaders(),
        body: form,
      });
      ...
      if (!finalResponse.ok || !finalStart?.ok || !finalStart?.taskId) {
        throw new Error(finalStart?.error || 'The final 3D voxel could not start.');
      }
  ...
  async function confirmAddress() {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data?.ok || !data?.available || !data?.atlasId || !data?.identityKey) {
        throw new Error(data?.error || 'That address could not be confirmed.');
      }
