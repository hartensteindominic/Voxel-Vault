...
export default function PropertyVoxelMintPage() {
  ...
  const modelUrl = useMemo(() => {
    if (params.modelUrl) return params.modelUrl;
    return params.taskId?.startsWith('local-v1:')
      ? `/api/property-local-voxel?taskId=${encodeURIComponent(params.taskId)}`
      : '';
  }, [params.modelUrl, params.taskId]);
  ...
  async function mint() {
    ...
    try {
      ...
      if (!confirm.ok || !verified?.verified) {
        throw new Error(verified?.error || `VoxelFlip #${result.tokenId} was submitted, but verification is not complete. Do not mint it again.`);
      }
  ...
  return <main className="page"><section className="shell">
    ...
    {invalid ? <section className="notice"><b>Open Mint from a finished VoxelPop 3D voxel.</b><Link href="/property">Create a voxel</Link></section> : <>
      ...
      <div className="viewer"><MeshyModelViewer modelUrl={modelUrl}/><span>{minted ? `VOXELFLIP #${minted.tokenId}` : 'YOUR FINAL 3D VOXEL'}</span></div>
      ...
      <details className="details"><summary>Mint details</summary><div className="facts"><div><small>NETWORK</small><b>Base</b></div><div><small>ASSET</small><b>Final 3D voxel</b></div><div><small>SOURCE PHOTO</small><b>Not in NFT metadata</b></div><div><small>PHYSICAL PROPERTY</small><b>No rights transferred</b></div></div><p>Your wallet approves the mint transaction. The one-time property voucher blocks a second NFT for the same confirmed property.</p></details>
