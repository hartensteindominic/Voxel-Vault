...
export default function PhotoReliefModelViewer({ imageUrl, onReady }) {
  ...
  useEffect(() => {
    ...
    image.onload = async () => {
      try {
        ...
        renderer.domElement.setAttribute('aria-label', 'Interactive 3D voxel photo. Drag or use the arrow keys to inspect shallow voxel depth while keeping the photographed house recognizable.');
        mount.innerHTML = '';
        mount.appendChild(renderer.domElement);

        const scene = new THREE.Scene();
        const camera = new THREE.PerspectiveCamera(28, initialWidth / initialHeight, 0.1, 80);
        const objectGroup = new THREE.Group();
        scene.add(objectGroup);

        scene.add(new THREE.HemisphereLight(0xfffcf4, 0x211529, 2.25));
        const key = new THREE.DirectionalLight(0xfff8ee, 3.1);
        key.position.set(4.6, 5.4, 7.4);
        key.castShadow = true;
        key.shadow.mapSize.set(compact ? 512 : 1024, compact ? 512 : 1024);
        scene.add(key);
        const fill = new THREE.DirectionalLight(0xdcd4ff, 1.25);
        fill.position.set(-4.2, 2.4, 4.6);
        scene.add(fill);
        const rim = new THREE.DirectionalLight(0xe8ffc0, 0.9);
        rim.position.set(2.2, 4.1, -3.4);
        scene.add(rim);
        ...
        // Higher sampling than the first VoxelPop relief keeps windows, roof lines,
        // doors and trim recognizable while every visible cell is still a real cube.
        ...
        const cubeMaterial = new THREE.MeshStandardMaterial({ roughness: 0.7, metalness: 0.02, vertexColors: true });
        ...
        for (let y = 0; y < rows; y += 1) {
          for (let x = 0; x < columns; x += 1) {
            ...
            const depth = 0.11 + (1 - luminance) * 0.17 + chroma * 0.055;
        ...
        // Keep a faint copy of the exact source under the cube gaps. This preserves
        // fine house identity without pretending the hidden sides were reconstructed.
        const texture = new THREE.Texture(image);
        texture.needsUpdate = true;
        texture.colorSpace = THREE.SRGBColorSpace;
        texture.minFilter = THREE.LinearFilter;
        texture.magFilter = THREE.LinearFilter;
        const referenceGeometry = new THREE.PlaneGeometry(photoWidth, photoHeight);
        const referenceMaterial = new THREE.MeshBasicMaterial({ map: texture, transparent: true, opacity: 0.34, depthWrite: false });
        const referencePlane = new THREE.Mesh(referenceGeometry, referenceMaterial);
        referencePlane.position.z = -0.075;
        objectGroup.add(referencePlane);

        const backingDepth = 0.12;
        const backingGeometry = new THREE.BoxGeometry(photoWidth + 0.13, photoHeight + 0.13, backingDepth);
        const backingMaterial = new THREE.MeshStandardMaterial({ color: 0x17101c, roughness: 0.76, metalness: 0.025 });
        ...
        const edgeMaterial = new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.17 });
        ...
        const groundMaterial = new THREE.ShadowMaterial({ color: 0x000000, transparent: true, opacity: compact ? 0.14 : 0.2 });
        const ground = new THREE.Mesh(groundGeometry, groundMaterial);
        ground.rotation.x = -Math.PI / 2;
        ground.position.set(0, -photoHeight / 2 - 0.48, 0.16);
        ...
        const fitCamera = (width, height) => {
          ...
          const distance = Math.max(distanceForHeight, distanceForWidth, 5.2) + 0.85;
          camera.position.set(0, 0.14, distance);
          camera.lookAt(0, -0.02, 0.04);
        };
        fitCamera(initialWidth, initialHeight);

        // Keep the inspection angle intentionally shallow: this stage is for checking
        // likeness to the source photo, not inventing a full building from unseen data.
        let targetX = -0.035;
        let targetY = 0.095;
        let pointerId = null;
        let lastX = 0;
        ...
        const move = (event) => {
          ...
          lastX = event.clientX;
          lastY = event.clientY;
          targetY = clamp(targetY + dx * 0.0044, -0.32, 0.32);
          targetX = clamp(targetX + dy * 0.0032, -0.13, 0.13);
        ...
        const keydown = (event) => {
          let handled = true;
          if (event.key === 'ArrowLeft') targetY = clamp(targetY - 0.045, -0.32, 0.32);
          else if (event.key === 'ArrowRight') targetY = clamp(targetY + 0.045, -0.32, 0.32);
          else if (event.key === 'ArrowUp') targetX = clamp(targetX - 0.035, -0.13, 0.13);
          else if (event.key === 'ArrowDown') targetX = clamp(targetX + 0.035, -0.13, 0.13);
          else handled = false;
          if (handled) {
  ...
  return <div className={`viewerShell ${styles.shell}`}>
    ...
    {!error ? <>
      <div className={styles.qualityBadge} aria-hidden="true"><span>3D VOXEL PHOTO</span><b>PHOTO-MATCHED VOXELS</b></div>
