...
function sampleRecipe(image) {
  ...
  // Keep the entire photo framing. The old square crop was the main reason a house
  // could stop looking like the uploaded building before voxelization even began.
  ...
  // If the photo has unusual lighting or a facade color close to the sky, relax
  // the actual-photo evidence. Never replace it with a fabricated generic house.
  ...
  if (!activeCount) throw new Error('VoxelPop could not isolate enough of the uploaded building to make a trustworthy voxel. Try a clearer front or three-quarter photo.');
...
export default function LocalVoxelModelViewer({ imageUrl, sourceImageUrl, onReady }) {
    ...
    image.onload = () => {
      ...
      import('three').then((THREE) => {
        ...
        try {
          ...
          setError('Interactive 3D is unavailable here. Your approved 3D picture remains safe.');
          return;
        }

        const initialWidth = Math.max(280, mount.clientWidth || 360);
        const initialHeight = Math.max(300, mount.clientHeight || 390);
        ...
        if (!dead) setError('Interactive voxel 3D could not start. Your approved 3D picture remains visible.');
      });
    };
    image.onerror = () => {
      if (!dead) setError('The approved property image could not be opened for voxel conversion.');
    };
  ...
  return <div className="viewerShell">
    {posterUrl ? <img className={`viewerPoster ${ready ? 'hidden' : ''}`} src={posterUrl} alt="Approved property preview"/> : null}
    <div ref={mountRef} className="viewerCanvas" aria-label="Interactive property voxel model"/>
    {!ready && !error ? <div className="viewerStage">APPROVED 3D PICTURE → BUILDING VOXELS</div> : null}
    {error ? <div className="viewerError">{error}</div> : null}
    <div className="viewerHint">{ready ? '3D VOXEL · DRAG BUILDING · PINCH TO ZOOM' : 'THE APPROVED HOUSE IMAGE STAYS VISIBLE WHILE VOXELS BUILD'}</div>
    <style jsx>{`
      .viewerShell{position:relative;width:100%;height:100%;min-height:300px;overflow:hidden;background:radial-gradient(circle at 50% 30%,#3c2a52,#18101f 66%)}
