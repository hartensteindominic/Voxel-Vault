...
function sampleRecipe(image) {
  ...
  // Preserve the complete source frame. A wide house must stay wide instead of
  // being center-cropped into the old square voxel grid.
  ...
  // Keep the central connected mass while allowing the real roofline width to
  // change from row to row. This prevents sky/yard from becoming solid voxels.
  ...
  if (activeCount < width * height * 0.16) {
    // Visual fallback only. It retains the rectangular source proportions and
    // never restores the old full square picture-wall.
    mask.fill(false);
...
export default function LocalVoxelModelViewer({ imageUrl, sourceImageUrl, onReady }) {
    ...
    image.onload = () => {
      ...
      import('three').then((THREE) => {
        ...
        try {
          ...
          setError('Interactive voxel 3D is unavailable here. The approved preview remains visible.');
          return;
        }
        ...
        if (!dead) setError('Interactive voxel 3D could not start. The approved preview remains visible.');
      });
    };
    image.onerror = () => {
  ...
  return <div className="localViewerShell">
    ...
    {!ready && !error ? <div className="stage">APPROVED 3D PREVIEW → BUILDING VOXEL</div> : null}
    {error ? <div className="softError">{error}</div> : null}
    <div className="hint">{ready ? 'DRAG BUILDING · PINCH TO ZOOM' : 'KEEPING HOUSE PROPORTIONS · LOCAL 3D'}</div>
    <style jsx>{`
