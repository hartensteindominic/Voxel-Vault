...
assert.match(photoPreview, /const depth = 0\.18/, '3D voxel photo has meaningful shallow block depth instead of a flat picture body');
assert.match(photoPreview, /edge \* 0\.24/, 'visible image edges influence Stage 3 depth');
assert.match(photoPreview, /voxels\.setColorAt\(instance, color\)/, 'voxel-photo colors remain tied to the source image');
assert.match(photoPreview, /PHOTO-MATCHED BLOCKS/, 'the review surface identifies real source-faithful blocks');
assert.match(photoPreview, /ORIGINAL PHOTO/, 'the original photo remains visible for a direct likeness check');
assert.doesNotMatch(photoPreview, /backingGeometry|referencePlane/, '3D voxel photo must not be mounted to a rectangular picture backing');
