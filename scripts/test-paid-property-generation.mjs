...
assert.match(route, /PropertyJourneyExact/, 'the /property route must use the strict voxel-photo -> movable voxel -> mint journey');
...
assert.match(property, /Pay \$\{CREATION_PRICE_LABEL\} & Make 3D Voxel Photo/, 'paid CTA promises the 3D voxel photo first');
...
assert.match(property, /PhotoReliefModelViewer imageUrl=\{pendingPreview\}/, 'paid flow shows the voxel-photo approval stage');
...
assert.match(photoPreview, /getImageData/, '3D voxel photo samples the uploaded image into voxel data');
assert.match(photoPreview, /new THREE\.InstancedMesh/, '3D voxel photo renders real block geometry');
assert.match(photoPreview, /BoxGeometry\(1, 1, 1\)/, '3D voxel photo is composed of cubes');
assert.match(photoPreview, /const depth = 0\.42/, '3D voxel photo has physical block depth rather than a flat backing body');
assert.match(photoPreview, /YOUR SOURCE PHOTO/, 'original photo remains visible for a direct likeness check');
assert.doesNotMatch(photoPreview, /backingGeometry/, 'the old picture-wall slab must stay removed from the voxel-photo stage');
...
assert.match(localStore, /saveCatalog3D/, 'account registration falls back to the shared catalog store if direct table persistence fails');
assert.match(localStore, /never the user's original house photo/i, 'fallback persistence must retain only recipe metadata, not the source house photo');
...
console.log('Paid VoxelPop regression passed: sign in -> photo -> one $4.99 payment -> real 3D voxel photo -> explicit approval -> separate movable local voxel -> resilient Vault save -> Mint Now or Mint Later, with no Meshy credits, hidden second paywall, or physical-property claim.');
