...
assert.match(property, /placeholder="Property address/, 'creator asks for the property identity before first checkout');
assert.match(property, /form\.append\('address', address\)/, 'creator passes the property address into the canonical one-property checkout');
assert.match(property, /block duplicate purchases and duplicate mints/i, 'creator explains why the property address is required');
assert.match(property, /One property can only be purchased once and minted once\./, 'creator clearly states the one-of-one rule before payment');
...
assert.match(checkout, /form\.get\('address'\)/, 'checkout accepts the explicit property address from the simple creator');
...
assert.match(checkout, /This property has already been purchased/, 'checkout blocks a permanently sold property before payment');
...
assert.match(paidVerify, /identityKey: receipt\.identityKey/, 'paid verification returns the canonical property identity to the creator');
assert.match(paidVerify, /propertyAddress: receipt\.propertyAddress/, 'paid verification restores the canonical property address after checkout');
...
assert.match(property, /identityKey: savedIdentityKey \|\| null/, 'saved Vault item retains its one-property identity');
...
assert.match(property, /One property, one purchase, one NFT maximum\./, 'completion reiterates the uniqueness invariant');
...
console.log('Paid VoxelPop regression passed: sign in -> one photo + property address -> canonical one-property lock -> one $4.99 payment -> real 3D voxel-photo review -> one approval -> automatic movable voxel -> automatic Vault save -> optional one-time mint.');
