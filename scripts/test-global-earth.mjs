...
assert.match(page, /GlobalEarthGlobe/, 'Earth UI must retain lightweight interactive globe.');
...
assert.match(globe, /SphereGeometry/, 'Global Earth should use lightweight Three.js stack.');
...
assert.match(globe, /pointerdown|pointermove|pointerup/, 'Globe must use pointer events for mouse/touch.');
...
console.log('Global Earth federation safety checks passed: worldwide navigation, authorized listings, source currencies, visible provider gaps, deed/title boundaries, and verification-first digital twins remain intact.');
