...
assert.match(photoPreview, /getImageData/, 'the first 3D stage samples colors from the uploaded source photo');
assert.match(photoPreview, /InstancedMesh/, 'the 3D voxel photo uses actual voxel instances');
assert.match(photoPreview, /BoxGeometry\(1, 1, 1\)/, 'the 3D voxel photo is built from block geometry');
assert.match(photoPreview, /const depth = 0\.11/, 'voxel-photo blocks keep shallow inspectable depth');
assert.match(photoPreview, /ORIGINAL PHOTO/, 'the original photo remains visible beside the voxel-photo preview');
...
console.log('Property journey regression passed: saved/reusable property photo or new photo -> one paid unlock -> 3D voxel photo -> explicit user approval -> separate movable local 3D voxel -> auto-save to Vault -> Mint Now or Mint Later, with optional map/World and no Meshy credits or second paywall.');
