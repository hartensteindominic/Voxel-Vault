...
assert.match(property, /PhotoReliefModelViewer/, '3D voxel photo is a distinct review stage');
assert.match(photoPreview, /getImageData/, '3D voxel photo samples the source image into voxel colors');
assert.match(photoPreview, /new THREE\.BoxGeometry\(1, 1, 1\)/, '3D voxel photo uses real cube cells');
assert.match(photoPreview, /InstancedMesh/, '3D voxel photo uses efficient real 3D geometry');
...
assert.match(viewer, /InstancedMesh/, 'movable voxel 3D uses actual Three.js voxel instances');
...
console.log('Property journey regression passed: saved/reusable property photo or new photo -> one paid unlock -> 3D voxel photo -> explicit user approval -> separate movable 3D voxel -> auto-save to Vault -> Mint Now or Mint Later, with optional map/World and no Meshy credits or second paywall.');
