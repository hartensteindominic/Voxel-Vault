...
const createPolish = read('app/property-create-polish.css');
...
assert.match(propertyRoute, /PropertyJourneyExact/, '/property must use the strict voxel-photo -> movable voxel -> mint journey');
assert.match(home, /ONE HOUSE PHOTO → VOXEL/, 'home keeps one photo as the simple starting point');
assert.match(home, /Upload one photo/, 'home describes the photo-first journey');
assert.match(home, /Create my VoxelPop · \$4\.99/, 'home clearly discloses the creation price on the primary action');
assert.match(home, /source photo stays on your device/i, 'home explains the device-local source photo boundary');
assert.match(home, /No Meshy credits are required/i, 'home makes the no-Meshy dependency explicit');
assert.match(home, /Photo → voxel photo → movable voxel → optional mint/i, 'home states the required creation order');
...
assert.match(home, /VoxelPop item or NFT is not a deed/i, 'home must separate digital items from real-property title');
...
assert.match(createPolish, /VOXEL PHOTO/, 'progress UI names the voxel-photo stage accurately');
assert.match(createPolish, /3D VOXEL/, 'progress UI distinguishes the separate movable voxel stage');
assert.doesNotMatch(createPolish, /3D PIC/, 'old ambiguous 3D-picture wording must not return');
...
assert.match(property, /Pay \$\{CREATION_PRICE_LABEL\} & Make 3D Voxel Photo/, 'paid CTA explicitly creates the voxel photo first');
...
assert.match(photoPreview, /getImageData/, 'voxel photo samples the uploaded photo into block colors and depth');
assert.match(photoPreview, /InstancedMesh/, 'voxel photo is made from real Three.js block geometry');
assert.match(photoPreview, /BoxGeometry\(1, 1, 1\)/, 'voxel photo uses individual cube geometry');
assert.match(photoPreview, /const depth = 0\.42/, 'voxel photo has meaningful physical block depth rather than a flat picture wall');
assert.match(photoPreview, /YOUR SOURCE PHOTO/, 'source photo remains visible for direct likeness comparison');
assert.doesNotMatch(photoPreview, /backingGeometry/, 'voxel photo must not restore the old dark picture backing slab');
assert.match(photoPreview, /targetY = clamp/, 'voxel-photo rotation stays deliberately bounded so unseen sides are not implied');
assert.match(property, /Looks good → Create Movable 3D Voxel/, 'user explicitly approves the voxel photo before movable voxelization');
...
assert.doesNotMatch(localViewer, /backingGeometry/, 'the movable voxel must not become a picture-wall slab');
...
assert.match(localStore, /saveCatalog3D/, 'local voxel persistence has the shared catalog fallback when the primary table cannot save');
assert.match(localStore, /never the user's original house photo/i, 'persistence fallback preserves the device-local source-photo boundary');
...
console.log('Guided VoxelPop property checks passed: sign in -> photo -> one $4.99 payment -> real 3D voxel photo -> explicit approval -> separate movable 3D voxel -> resilient Vault save -> Mint Now or Mint Later, with map/World optional and regulated/property-rights rails distinct.');
