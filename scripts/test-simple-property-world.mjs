...
assert.match(home, /Upload one property photo\./, 'home describes the photo-first journey');
assert.match(home, /Create mine · \$4\.99/, 'home clearly discloses the creation price');
assert.match(home, /Photo stays on your device/i, 'home explains the device-local source photo boundary');
assert.match(home, /No wallet required to create/i, 'wallet must not block core creation');
assert.match(home, /REVIEW 3D VOXEL PHOTO[\s\S]*CREATE MOVABLE VOXEL/i, 'home states the required voxel-photo -> movable-voxel order');
assert.match(home, /Mint only if you want/i, 'home makes mint optional');
...
assert.match(homeCss, /#fbfaf8/i, 'home keeps a warm neutral VoxelPop canvas');
assert.match(propertyCss, /#f8f7f5/i, 'Create keeps the warm neutral VoxelPop canvas');
for (const source of [vault, world]) assert.match(source, /#fffaf0|#fffaf2/i, 'World and Vault keep the established warm VoxelPop canvas');
assert.match(propertyCss, /#6f3df4/i, 'VoxelPop purple remains');
assert.match(propertyCss, /#c9ff54/i, 'VoxelPop lime remains');
assert.match(propertyCss, /#f5a524/i, 'warm accent remains available');
assert.match(propertyCss, /grid-template-columns:repeat\(5,1fr\)/, 'maker keeps five guided steps');
assert.match(propertyCss, /aspect-ratio:4\/3/, 'house review stays landscape-oriented');
...
assert.match(property, /You see and approve the 3D voxel photo before VoxelPop creates the separate movable 3D voxel/, 'one payment preserves the voxel-photo-before-movable-voxel order');

assert.match(property, /indexedDB\.open\(DEVICE_DB/, 'source photo is kept privately on-device across checkout');
assert.match(property, /PhotoReliefModelViewer/, '3D voxel photo review is a first-class stage');
assert.match(photoPreview, /getImageData/, 'first 3D stage samples the visible source photo into voxel colors');
assert.match(photoPreview, /new THREE\.BoxGeometry\(1, 1, 1\)/, 'first 3D stage is made from real cube cells');
assert.match(photoPreview, /InstancedMesh/, 'first 3D stage uses actual Three.js voxel geometry');
assert.match(photoPreview, /ORIGINAL PHOTO/, 'original photo stays visible for direct likeness comparison');
assert.match(photoPreview, /targetY = clamp/, '3D voxel photo rotation stays deliberately bounded so unseen sides are not invented');
assert.match(property, /Looks good → Create Movable 3D Voxel/, 'user explicitly approves the voxel photo before the movable model');
assert.match(property, /createVoxelPoster/, 'movable-voxel preparation starts only after voxel-photo approval');
...
assert.match(property, /Minting is optional/, 'finished state says minting is optional');
...
console.log('Guided VoxelPop property checks passed: sign in -> photo -> one $4.99 payment -> 3D voxel photo -> approval -> separate higher-detail movable voxel -> auto-save to Vault -> optional Mint Now or Mint Later, with map/World optional and regulated/property-rights rails distinct.');
