...
const photoPreview = read('app/property/PhotoDepthPreview.js');
...
assert.match(propertyCss, /grid-template-columns:repeat\(6,1fr\)/, 'maker exposes six understandable guided stages');
assert.match(propertyCss, /object-fit:contain/, 'property source imagery must not be forced into a square crop');
...
assert.match(property, /const labels = \['PHOTO', 'PAY', '3D PREVIEW', 'VOXEL', 'MAP', 'SAVE \+ MINT'\]/, 'labels explain the preview-before-voxel journey');
assert.match(property, /Start with the real house photo\./, 'first signed-in step is photo-first');
...
assert.match(property, /Pay \$\{CREATION_PRICE_LABEL\} · Open 3D Preview/, 'paid CTA clearly opens the 3D preview before voxelization');
assert.match(property, /purchase unlocks the 3D photo preview and the voxel version/i, 'one payment includes both useful visual stages');
assert.match(property, /there is no second charge/i, 'a second creation payment cannot block the normal journey');
assert.match(property, /PhotoDepthPreview imageUrl=\{pendingPreview\}/, 'photo-faithful preview is a separate stage');
assert.match(property, /3D PHOTO PREVIEW · NOT VOXEL YET/, 'preview stage cannot be confused with the voxel');
assert.match(property, /Looks right · Create Voxel 3D/, 'voxelization waits for user approval');
assert.doesNotMatch(property, /await startVoxelBuild\(/, 'payment return cannot silently skip preview approval');

assert.match(property, /indexedDB\.open\(DEVICE_DB/, 'source photo can be kept privately on-device across checkout');
assert.match(property, /let cachedOnDevice = false/, 'on-device caching is best-effort rather than a checkout dependency');
assert.match(property, /browser could not keep the photo privately through checkout/i, 'storage failure has an explicit non-blocking recovery path');
assert.match(photoPreview, /new THREE\.Texture\(image\)/, 'the first 3D view uses the actual source photo');
assert.match(photoPreview, /PlaneGeometry/, 'the first 3D view has real interactive relief geometry');
assert.match(photoPreview, /Depth is estimated locally/, 'single-photo depth limitations remain visible');
assert.match(property, /createVoxelPoster/, 'VoxelPop voxel image is built locally only after preview approval');
assert.match(property, /LocalVoxelModelViewer/, 'local interactive voxel replaces provider generation');
assert.match(localViewer, /const MAX_SIDE = 24/, 'building voxel stays within the local server recipe limit');
assert.match(localViewer, /gridForImage/, 'building voxel preserves the source aspect ratio');
...
assert.match(localViewer, />SOURCE<\//, 'voxel can be compared with its original source');
assert.match(localViewer, /object-fit:contain/, 'source comparison avoids square cropping');
...
assert.match(property, /Match this voxel to the real map/, 'address follows successful local voxel creation');
...
assert.match(property, /Verify \+ Mint · Optional/, 'completed creation offers minting only after the digital voxel is saved');
...
console.log('Guided VoxelPop property checks passed: sign in -> photo -> one $4.99 payment -> photo-faithful 3D preview -> user-approved voxel -> source-backed map -> save/view -> optional mint, while the separate optional collectible product remains distinct and non-blocking.');
