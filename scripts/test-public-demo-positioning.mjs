...
assert.match(photoViewer, /const longSide = compact \? 64 : 88/, 'voxel-photo likeness review must use the denser source grid');
assert.match(photoViewer, /dummy\.position\.set\(xPos, yPos, -depth \* 0\.5\)/, 'all voxel front faces must share the source-photo plane while depth extends backward');
assert.match(photoViewer, /THREE\.NoToneMapping/, 'likeness review must not introduce cinematic tone-map shifts');
assert.match(photoViewer, /THREE\.SRGBColorSpace/, 'source-photo colors must keep an explicit sRGB path');
assert.match(photoViewer, /new THREE\.MeshBasicMaterial/, 'photo-match faces must stay unlit so the approved colors are not cosmetically relit');
...
assert.match(photoViewer, /ORIGINAL PHOTO · COMPARE/, 'voxel-photo preview must keep the original source visible for direct comparison');
assert.match(photoViewer, /PHOTO-MATCHED FRONT/, 'viewer must identify the source-faithful front as the likeness reference');
assert.match(photoViewer, /RESET FRONT/, 'viewer must let the user return to the source-matched approval view after inspection');
assert.match(photoViewer, /targetY = clamp\(targetY \+ dx \* 0\.0031, -0\.22, 0\.22\)/, 'single-photo rotation must remain tightly bounded around the known front view');
...
assert.match(photoViewerStyles, /width:138px/, 'desktop likeness review must keep a large original-photo comparison card');
...
console.log('Public VoxelPop positioning checks passed: sample-first product proof, source-plane photo-matched 3D voxel geometry without a backing or plaque, explicit reset/compare controls, movable voxel separation, focused $4.99 story, corrected trust pages, and current social preview remain aligned.');
