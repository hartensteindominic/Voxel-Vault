...
const entries = buildVaultManifest({
  ...
  collectibles: [
    {
      tokenId: '9',
      metadata: { name: 'Verified Twin #9', image: 'ipfs://image' },
    },
  ],
  reitPositions: [
...
const malformed = buildVaultManifest({
  creations: [{ sessionId: '', image: 'x' }, { sessionId: 'x', image: '' }],
  collectibles: [{ tokenId: '' }],
  reitPositions: [{ symbol: 'BAD', amount: 'not-a-number' }],
});
