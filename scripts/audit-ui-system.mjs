...
must(/className=\{styles\.secondaryAction\} href="\/demo"/.test(home), 'No-login demo must remain a secondary proof action, not primary navigation.');
must(/One simple flow/i.test(home), 'Homepage must state the condensed one-flow product promise.');
must(/VOXELPOP OUTPUT/.test(home) && /3D voxel photo/i.test(home) && /Movable 3D voxel/.test(home) && /Optional NFT/.test(home), 'Homepage must preserve the real voxel-photo, movable-voxel, and optional-NFT product truth.');
...
must(/Create · \$4\.99[\s\S]*Vault/.test(topNav), 'Focused product header must contain Create and Vault.');
must(!/label: 'World'/.test(topNav) && !/className=\{styles\.demo\}/.test(topNav), 'World and Demo must stay out of the primary product header.');
...
must(/FinancialOSNav\.module\.css/.test(dock), 'Bottom dock must use the responsive stylesheet rather than always-on inline chrome.');

must(/SimpleJourneyPresentation/.test(propertyPage), 'Property creator must apply the one-action presentation layer.');
must(/Choose a property photo/.test(propertyPage) && /data-vv-hide-duplicate/.test(propertyPage), 'Creator simplification must hide only the duplicate custom photo target while retaining the real photo button.');
must(/Done · Open Vault/.test(propertyPage) && /data-vv-primary-finish/.test(propertyPage), 'Completed creation must make Done/Open Vault the primary finish action.');
must(/Mint Now/.test(propertyPage) && /data-vv-secondary-action/.test(propertyPage), 'Mint Now must remain available but visually secondary.');
...
if (failures.length) {
  ...
  console.log('\nUI system invariants passed: real 3D proof, one-flow homepage, Create/Vault header, Home/VoxelPop/Vault mobile dock, one-action creator presentation, readable trust chrome, optional minting, focus visibility, and reduced-motion support are enforced.');
}
