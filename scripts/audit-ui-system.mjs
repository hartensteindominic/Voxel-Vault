...
must(/HomeProductPreview/.test(home), 'Homepage must use real production visual proof.');
must(!/voxelHouse/.test(home), 'Homepage must not regress to decorative CSS art.');
must(/className=\{styles\.primaryAction\} href="\/property"/.test(home), 'Create must be the single visual primary hero action.');
must(/className=\{styles\.secondaryAction\} href="\/demo"/.test(home), 'No-login sample must be the secondary proof action.');
must(/WHAT YOU GET/.test(home), 'Homepage must keep the product outputs easy to scan.');
must(/SAVE \/ OPTIONAL MINT/.test(home), 'Homepage must keep minting downstream and optional.');
must(/3D voxel photo/.test(home) && /movable 3D voxel/i.test(home), 'Homepage must distinguish the voxel-photo review from the movable model.');
must(/PhotoReliefModelViewer/.test(preview) && /LocalVoxelModelViewer/.test(preview), 'Home product proof must use the production voxel-photo and movable-voxel viewers.');
...
must(!/box-shadow:0 6px 0 #5120d0/.test(homeCss), 'Homepage CTA must not regress to an exaggerated toy-like extrusion.');
...
must(/href="\/demo"/.test(topNav), 'Desktop product nav must keep the public sample reachable.');
...
must(/getImageData/.test(photoViewer), '3D voxel photo must sample the selected photo instead of displaying a flat image.');
must(/InstancedMesh/.test(photoViewer) && /BoxGeometry\(1, 1, 1\)/.test(photoViewer), '3D voxel photo must remain real cube geometry.');
must(/const columns = compact \? 52 : 64/.test(photoViewer), '3D voxel photo must keep the denser likeness-preserving grid.');
must(/-0\.28, 0\.28/.test(photoViewer), '3D voxel photo rotation must stay bounded so hidden sides are not invented.');
...
must(/\/demo/.test(footer) && /\/privacy/.test(footer) && /\/about/.test(footer), 'Shared footer must cover sample and trust surfaces.');
...
if (failures.length) {
  ...
  console.log('\nUI system invariants passed: faithful real 3D voxel-photo proof, condensed navigation, landscape Create review, prominent source comparison, optional minting, focus visibility, and reduced-motion support are enforced.');
}
