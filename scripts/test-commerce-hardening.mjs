...
const mintVerification = fs.readFileSync('lib/base-mint-verification.ts', 'utf8');
const verifyMintRoute = fs.readFileSync('app/api/admin/products/[id]/verify-mint/route.ts', 'utf8');
...
const required = [
  ...
  ['readiness-only admin actions', /\['save', 'review'\]/g, productUpdate],
  ...
  ['database publication gate', /PRODUCT_NOT_VAULT_READY/g, productDraftMigration],
  ['Base transaction receipt verification', /eth_getTransactionReceipt/g, mintVerification],
  ['ERC-721 transfer log verification', /TRANSFER_TOPIC/g, mintVerification],
  ['server-side mint verification route', /verifyBaseMint/g, verifyMintRoute],
