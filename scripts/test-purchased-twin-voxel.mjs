...
assert.match(more, /Create → World → Vault/, 'More must reinforce the simple main journey.');
assert.match(more, /My purchased twins/, 'More must expose the purchased-twin destination without hiding it in a giant directory.');
assert.match(more, /Bought or saved a property\?/, 'More must keep the reusable property workflow from the latest main branch.');
assert.match(more, /3D preview → approve → 3D voxel → optional mint/, 'More must state the real property-creation order.');
...
assert.match(maker, /PhotoReliefModelViewer/, 'The purchased flow must show the 3D preview before voxelization.');
...
console.log('Purchased Digital Twin flow passed: latest simple Vault/More -> account-verified purchase -> included 3D preview -> local voxel -> exact saved reopen -> optional mint.');