...
// one no-login sample, the actual photo -> voxel photo -> movable voxel -> optional NFT order,
// and a visible digital-only rights boundary. Advanced property/financial tooling stays off it.
assert.match(rootHome, /VOXELPOP · PHOTO → 3D VOXEL PHOTO → MOVABLE VOXEL → NFT/, 'root Home must state the exact centered product sequence');
...
assert.match(rootHome, /3D voxel photo/, 'root Home must name the intermediate output as a voxel photo');
