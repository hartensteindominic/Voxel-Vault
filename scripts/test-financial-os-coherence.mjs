...
for (const label of ['Home', 'Create', 'Vault', 'Rented', 'World']) {
  assert.match(productMap, new RegExp(`label: '${label}'`), `simple property dock should include ${label}`);
}
assert.match(productMap, /label: '\$1\.99'/, 'simple property dock should keep the $1.99 demo shortcut');
...
assert.match(productMap, /Authorized photo → \$4\.99 local voxel creation/, 'product map should describe the actual paid local VoxelPop path');
assert.match(productMap, /fake demo USD/i, '$1.99 must stay explicitly demo-only');
...
assert.doesNotMatch(vaultLayout, /financial OS/i, 'Vault metadata should not market Voxel Vault as a financial institution-like OS');
...
// Root Home must explain the current product before exposing advanced finance concepts.
...
assert.match(rootHome, /CREATE FOR \$4\.99/);
assert.match(rootHome, /START → SIGN IN/);
assert.match(rootHome, /PHOTO/);
assert.match(rootHome, /\$4\.99/);
assert.match(rootHome, /VOXEL 3D/);
assert.match(rootHome, /WORLD/);
assert.match(rootHome, /OPTIONAL COLLECT/);
assert.match(rootHome, /No Meshy credits are used/);
assert.match(rootHome, /source photo stays on the device/i);
assert.match(rootHome, /separate digital-collectible checkout/i);
...
for (const status of ['LIVE DIGITAL', 'DEMO', 'PARTNER REQUIRED', 'TITLE REQUIRED']) assert.match(rootHome, new RegExp(status));
...
for (const status of ['LIVE DIGITAL', 'DEMO', 'PARTNER REQUIRED', 'TITLE REQUIRED']) assert.match(realEstate, new RegExp(status));
...
for (const status of ['LIVE DIGITAL', 'DEMO', 'PARTNER REQUIRED', 'TITLE REQUIRED']) assert.match(readme, new RegExp(status));
...
console.log('Voxel Vault condensed product truth + paid local VoxelPop + real-estate status regression tests passed');
