...
// One canonical product map keeps the consumer app organized. The simplified
// top navigation may show fewer destinations, but every route still maps into
// Home/Create/World/Vault/More at the app-shell level.
...
assert.match(productMap, /\$1\.99 Property Sandbox/, 'the tiny property comparison must remain explicitly sandboxed under advanced tools');
assert.match(productMap, /No real funds or property rights move/, 'sandbox description must preserve the no-rights boundary');
assert.match(productMap, /Authorized photo → \$4\.99 real 3D voxel photo → approval → separate movable 3D voxel/, 'property creator directory entry must disclose the real voxel-photo-before-model sequence');
...
assert.match(layout, /Turn a House Photo into a 3D Voxel Photo/, 'public metadata should describe the current product rather than internal architecture jargon');
...
// Consumer Home is intentionally focused on one creation story.
assert.match(rootHome, /VOXELPOP · ONE HOUSE PHOTO · \$4\.99/, 'root Home should present one photo and one creation price');
assert.match(rootHome, /href="\/demo"/, 'root Home should let visitors inspect the free sample before sign-in');
assert.match(rootHome, /href="\/property"/, 'root Home should route creation into the account-gated maker');
assert.match(rootHome, /Choose one house photo/, 'root Home should keep one photo as the obvious creation source');
...
assert.match(rootHome, /HomeProductPreview/, 'root Home should prove the product with the real production visual stages');
assert.match(homePreview, /label: 'House photo'/, 'home product proof should include the source-photo stage');
assert.match(homePreview, /label: '3D voxel photo'/, 'home product proof should include the real 3D voxel-photo stage');
assert.match(homePreview, /label: 'Movable 3D voxel'/, 'home product proof should distinguish the later movable model');
assert.match(rootHome, /1 · PHOTO/, 'root Home should show the photo stage');
assert.match(rootHome, /2 · 3D VOXEL PHOTO/, 'root Home should expose the voxel-photo stage before the model');
assert.match(rootHome, /3 · MOVABLE VOXEL/, 'root Home should expose the separate movable-voxel stage');
assert.match(rootHome, /4 · SAVE \/ OPTIONAL MINT/, 'root Home should make saving primary and minting optional');
...
assert.match(rootHome, /does not create ownership or financial rights in a physical property/i, 'root Home must distinguish the digital product from physical-property rights');
...
// Extras keeps the core app short while preserving optional destinations.
assert.match(more, /Keep VoxelPop simple\./i, 'Extras must explain its simplified purpose');
assert.match(more, /main product is Create → 3D voxel photo → movable voxel → Vault/i, 'Extras must reinforce the actual core product');
assert.match(more, /Making a house voxel\?/i, 'Extras must lead with the reusable house-voxel workflow');
assert.match(more, /review the <b>3D voxel photo<\/b> before the separate movable model is built/i, 'Extras must state the actual current creation order');
assert.match(more, /VOXEL PHOTO[\s\S]*APPROVE[\s\S]*MOVABLE VOXEL/, 'Extras visual flow must distinguish the two 3D stages');
assert.match(more, /Create VoxelPop · \$4\.99 →/, 'Extras must return users directly to the core creation flow');
assert.match(more, /VoxelPop demo/, 'Extras must keep the free sample easy to reach');
assert.match(more, /OWNER \/ PROVIDER TOOLS/, 'provider and legal rails must stay visibly separated');
assert.match(more, /not part of the \$4\.99 VoxelPop product/i, 'advanced provider tools must not masquerade as part of the consumer purchase');
...
// Detailed real-estate remains advanced, status-driven, and fail-closed without finance-dashboard theater.
...
console.log('Voxel Vault coherence checks passed: focused photo -> $4.99 -> real 3D voxel photo -> approve -> movable voxel -> Vault/optional mint, plus unambiguous sandbox and fail-closed advanced rails.');
